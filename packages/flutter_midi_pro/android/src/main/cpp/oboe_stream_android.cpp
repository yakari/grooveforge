...
/// instead of AAudio.
///
/// While set, the AAudio stream is kept closed: oboe_stream_start() and the
/// watchdog leave it alone, so the bus is only ever rendered from one thread.
/// g_streamWanted keeps tracking what the app asked for, so that when the
/// external clock goes away AAudio comes back exactly as it would have been.
...
static void startStreamWatchdog()
{
    ...
    std::thread([]() {
        while (g_streamWanted.load(std::memory_order_relaxed)) {
            ...
            // Down on purpose: the direct USB output is the clock right now.
            if (g_externalClock.load(std::memory_order_relaxed)) continue;
...
extern "C" void oboe_stream_start(int sampleRate)
{
    ...
    // The direct USB output is rendering the bus. Opening AAudio now would put
    // a second thread on the same single-threaded buffers; the stream comes
    // back through oboe_stream_end_external_clock() instead.
...
extern "C" void oboe_stream_set_output_device(int deviceId)
{
    ...
    //
    // It used to be, on the same "new hardware, new chance" reasoning. That
    // reasoning is wrong, because most device changes during a fault are not
    // new hardware — they are the audio server churning its device list as it
    // restarts, and the restart was caused by us. Re-arming on them makes the
    // crash feed itself:
    //
    //     14:45:14  stream dies young -> low latency denied (correct)
    //     14:45:19  settled on the mixer path, playing
    //     14:46:26  "device changed 0 -> 30"  -> denial cleared -> MMAP open
    //               -> audioserver aborts again
    //     14:46:27  "device changed 30 -> 0"  -> denial cleared -> aborts again
    //     14:46:35  finally back on the mixer path
    //
    // Nobody touched the hardware in that minute and a half. Neither id was
    // new: 0 is "system default", which on that phone *is* device 30. The
    // whole outage was this line handing the fast path back to a platform that
    // had just proved it aborts on it.
    //
    // Once earned, the denial therefore stands for the session. Measured on a
    // Galaxy Fold 6 with a USB-C DAC, nothing recovers it from inside the
    // process: retries after 20 s, 60 s and 180 s of an undisturbed stream all
    // aborted the audio server within 200 ms, and so did both device changes
    // above. An app restart clears it, which is the honest escape hatch.
    //
    // The cost is real and accepted: degrade on the DAC, then switch to the
    // speaker, and that output runs at mixer latency until the app is
    // restarted. That is one instrument feeling sluggish. The behaviour it
    // replaces silenced every app on the phone, repeatedly.
    LOGI("Output device changed: %d -> %d%s", previous, deviceId,
         g_lowLatencyDenied.load(std::memory_order_relaxed)
                 ? " (staying off the low-latency path — it aborted this "
                   "device's audio server earlier this session)"
                 : "");
...
extern "C" void oboe_stream_begin_external_clock(int32_t sampleRate)
{
    ...
    // Close rather than stop: oboe_stream_stop() would also record that the
    // app no longer wants audio, and the watchdog would then never bring
    // AAudio back after the USB output ends. AAudioStream_close() returns only
    // once the callback thread has finished, so the USB thread that starts
    // after this call is the bus's only renderer.
    if (g_stream != nullptr) {
        AAudioStream_requestStop(g_stream);
        AAudioStream_close(g_stream);
        g_stream = nullptr;
        LOGI("AAudio stream closed — the direct USB output is the clock now");
    }
...
extern "C" void oboe_stream_end_external_clock(void)
{
    ...
    // Reopen on a detached thread: opening AAudio can block for seconds while
    // the platform settles a route, and the caller is either the USB thread on
    // its way out or the Dart-facing stop, neither of which should wait.
...
extern "C" int32_t oboe_stream_get_routed_device_id(void)
{
    if (g_externalClock.load(std::memory_order_relaxed)) {
        return OBOE_ROUTED_DEVICE_USB_DIRECT;
    }
    // Under the stream mutex: the stream may be closed and reopened by a
    // route change on another thread while this is being read.
    std::lock_guard<std::mutex> lock(g_streamMtx);
    if (g_stream == nullptr) return 0;
    return AAudioStream_getDeviceId(g_stream);
}
