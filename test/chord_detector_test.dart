...
void main() {
  group('ChordDetector Tests', () {
    test('Identifies C Major', () {
      expect(ChordDetector.identifyChord({60, 64, 67})?.name, 'C'); // Root position
      expect(ChordDetector.identifyChord({64, 67, 72})?.name, 'C/E'); // 1st inversion
      expect(ChordDetector.identifyChord({67, 72, 76})?.name, 'C/G'); // 2nd inversion
    });
    ...
    test('Identifies add9 and other extensions', () {
      ...
      expect(ChordDetector.identifyChord({60, 64, 67, 71, 74})?.name, 'Cmaj7(9)');
    ...
    test('Identifies complex jazz chords', () {
      ...
      expect(ChordDetector.identifyChord({54, 60, 64, 70, 78})?.name, 'F#7(#11)');
    });
    ...
    test('Identifies 11th and 13th chords explicitly', () {
      ...
      expect(ChordDetector.identifyChord({60, 63, 67, 70, 77})?.name, 'Cm7(11)');
      // Cm13 (Root=C(60), m3=Eb(63), P5=G(67), m7=Bb(70), M13=A(81))
      expect(ChordDetector.identifyChord({60, 63, 67, 70, 81})?.name, 'Cm7(13)');
    });

    test('Identifies chords with solfege notation correctly', () {
      // C Major (Do)
      expect(ChordDetector.identifyChord({60, 64, 67}, format: NotationFormat.solfege)?.name, 'Do ');
      expect(ChordDetector.identifyChord({64, 67, 72}, format: NotationFormat.solfege)?.name, 'Do /Mi ');
      
      // F# Diminished (Fa#dim)
      expect(ChordDetector.identifyChord({66, 69, 72}, format: NotationFormat.solfege)?.name, 'Fa# dim');
      
      // Bb/F (Sib/Fa)
      expect(ChordDetector.identifyChord({53, 58, 62, 65}, format: NotationFormat.solfege)?.name, 'Sib /Fa ');
    });
  });
}
