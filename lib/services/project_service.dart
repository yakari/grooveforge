...
class ProjectService extends ChangeNotifier {
  ...
  ///
  /// The actual FFI plumbing (reading native clip buffers, writing the WAV
  /// header + PCM, cleaning up orphans) lives in
  /// [VstHostService.exportAudioLooperWavs], which this method reaches via
  /// [AudioLooperEngine.wavExporter] (set up by `SplashScreen`).  Keeping
  /// the exporter behind a callback means `project_service.dart` does NOT
  /// import `dart:ffi`, so the `dart2js` / `dart2wasm` web build compiles
  /// cleanly — `vst_host_service.dart` already uses a conditional export to
  /// swap in a no-op stub on web, and `dart:ffi` stays confined to the
  /// native side of that split.
  Future<void> _exportAudioLooperWavs(String gfPath) async {
    ...
    final exporter = engine.wavExporter;
    if (exporter == null) return;
    await exporter(gfPath, engine.clips);
  }
