...
class _GFpaThereminSlotUIState extends State<GFpaThereminSlotUI>
  ...
  void initState() {
    ...
    // Native oscillator bring-up + Android AAudio bus registration are owned
    // by [NativeInstrumentController], keyed to the slot's presence in the
    // rack — not this widget's mount status. The rack is a lazy
    // [ReorderableListView.builder], so tying the native synth to the widget
    // would silence the theremin whenever the slot scrolled off-screen. See
  ...
  void dispose() {
    ...
    _stopCurrentNote();
    // Note: we do NOT call thereminStop() / bus-remove here. The native
    // oscillator lifetime belongs to the rack slot (via
    // [NativeInstrumentController]), not to this widget. Stopping on widget
    // dispose would silence the theremin whenever the lazy list scrolled
    // the slot off-screen.
