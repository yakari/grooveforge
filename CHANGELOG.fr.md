...
### Modifié
- Audio Harmonizer : une prise **CHORD IN**. Reliez-y le CHORD OUT d'un clavier et l'accord joué règle directement les voix — la voix 1 prend la première note de l'accord au-dessus de la note chantée, la voix 2 la suivante, etc. — au lieu de conserver les intervalles réglés. L'accord est maintenu quand vous relâchez les touches : l'harmonie reste en place entre les phrases, et les lignes de voix sont grisées puisque c'est l'accord qui les règle. Un accord de trois notes n'en voise que trois, quel que soit le réglage Voices, qui agit donc comme un plafond plutôt que de doubler une note au hasard. Un accord câblé prend le pas sur le Scale Lock, étant l'instruction la plus précise.
...
- Android : brancher un DAC USB-C pouvait couper le son de toutes les applications du téléphone jusqu'au redémarrage. Router un flux low-latency (MMAP) vers une sortie USB fait carrément planter le serveur audio de cet appareil — son compteur de dose sonore prend deux verrous dans le mauvais ordre, et le constructeur livre cette vérification en assertion fatale — donc GrooveForge détecte maintenant la boucle de plantage et rouvre le flux sur le chemin audio classique, qui n'a pas de thread MMAP pour la déclencher. La latence augmente sur cette sortie uniquement, et le chemin rapide est retenté à chaque changement de périphérique de sortie.
...
### Architecture
- `ChordMatch` expose désormais les notes propres de l'accord en plus de la gamme qu'il implique. Les deux ne sont pas interchangeables pour un harmonizer : aligner une voix sur la gamme implicite peut la poser sur une note de passage qui jure avec l'accord réellement joué, ce que l'alignement sur les notes de l'accord ne peut pas faire.
