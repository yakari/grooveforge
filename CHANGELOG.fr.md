...
### Ajouté
- **MIDI OUT pour le Thérémine et le Stylophone** : les deux instruments disposent désormais d'une prise **MIDI OUT** dans la vue arrière du rack. Branchez un câble vers un GF Keyboard, un VST3 ou un MIDI Looper pour le piloter depuis le Thérémine ou le Stylophone. Le Thérémine détecte les changements de demi-ton sur la hauteur continue et envoie note-on/off à chaque franchissement. Le Stylophone envoie un note-on à chaque touche pressée et un note-off au relâchement.
- **Bouton MUTE** sur le Thérémine (dans la rangée PAD/CAM) et sur le Stylophone (à côté du bouton VIB) : coupe le synthétiseur C natif intégré tout en laissant les événements MIDI OUT continuer à circuler — idéal pour les utiliser comme contrôleurs MIDI expressifs sans doubler le son.
- **Correction de la rotation de l'aperçu caméra Android** : la vignette semi-transparente du mode CAM du Thérémine tient maintenant compte de l'orientation d'affichage de l'appareil pour apparaître à l'endroit en mode portrait et en paysage.
- **Hauteur du pad du Thérémine personnalisable** : un nouveau contrôle **HAUTEUR** dans la barre latérale du Thérémine permet de choisir entre quatre tailles — S (120 px), M (160 px), L (210 px), XL (270 px). La valeur par défaut est désormais M, légèrement plus haute qu'avant. La taille choisie est sauvegardée dans le fichier projet.
- **Bouton VIB (vibrato du Stylophone)** : appuyez sur le bouton VIB sous le sélecteur de forme d'onde pour activer un LFO à 5,5 Hz qui module la hauteur de ±0,5 demi-ton, reproduisant l'effet de « tape-wobble » des Stylophones vintage. L'état du vibrato (activé à 0,7 ou désactivé) est sauvegardé dans le fichier projet.
- **Verrouillage du défilement sur le pad du Thérémine** : le pad tactile du Thérémine s'approprie désormais tous les gestes de panoramique, empêchant le défilement accidentel du rack pendant que le joueur est sur la surface.
- **Fallback par contraste pour les caméras à focale fixe** : le mode CAM du Thérémine ne retourne plus d'erreur `FIXED_FOCUS` sur les appareils sans autofocus. Il bascule automatiquement vers une analyse de la luminosité/contraste de la zone centrale (50%) de chaque image, avec une normalisation min/max glissante sur 60 images s'adaptant aux conditions d'éclairage ambiant. Fonctionne sur tous les appareils Android et webcams.
- **Aperçu caméra dans le mode CAM du Thérémine** : un aperçu en niveaux de gris semi-transparent (opacité 0,35) à 5 ips est affiché derrière l'orbe lumineux, offrant une vue en direct de ce que voit la caméra (Android, iOS, macOS).
...
- **Mode CAM du Thérémine** (Android / iOS / macOS) : appuyez sur le bouton **CAM** au-dessus du pad pour passer du jeu tactile au jeu par caméra. La distance focale de l'autofocus de la caméra avant mesure la proximité de la main — rapprochez la main pour monter la hauteur, éloignez-la pour la descendre. Un filtre EMA (α = 0,15) lisse les lectures brutes image par image pour une réponse musicale fluide. Un orbe lumineux suit la position calculée. Le volume reste au maximum dans ce mode. Dégradation gracieuse : les caméras à focale fixe, les permissions refusées et les plateformes non prises en charge (Linux, Windows, web) affichent un message d'erreur localisé ; le bouton CAM est grisé avec une infobulle explicative.
...
- **Synthétiseur natif C pour le Stylophone** : remplace FluidSynth par un oscillateur monophonique dédié tournant dans son propre périphérique miniaudio. Quatre formes d'onde sélectionnables dans le slot — SQR (carré, bourdonnement classique du Stylophone), SAW (en dents de scie), SIN (sinusoïde), TRI (triangulaire). La phase est préservée entre les glissements de touches pour un legato sans clic. Court envelope de relâchement (τ ≈ 104 ms) pour éviter les clics en fin de note. La forme d'onde sélectionnée est sauvegardée dans le fichier projet.
- **Synthétiseur natif C pour le Thérémine** : remplace FluidSynth par un oscillateur sinusoïdal dédié tournant dans son propre périphérique miniaudio. Portamento fluide (τ ≈ 42 ms), LFO vibrato à 6,5 Hz avec profondeur réglable (0–100 % par pas de 25 % via les boutons +/− du slot), et légère 3e harmonique (+10 %) pour le timbre. La hauteur est envoyée en Hz directs (pas de notes MIDI échelonnées) pour un glissé continu. La profondeur du vibrato est sauvegardée dans le fichier projet.
- Contrôle du vibrato ajouté dans la barre latérale du Thérémine (label VIB, boutons +/−, valeurs 0 %/25 %/50 %/75 %/100 %).
- Le mode caméra du Thérémine envoie désormais le volume natif plein (1,0) plutôt qu'un CC MIDI 7.
