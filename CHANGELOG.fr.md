...
### Corrigé
- **Plantage macOS (corruption de tas / EXC_BAD_ACCESS dans `shared_preferences_foundation`)** : `g_slice` de glib contourne la zone malloc système et corrompt les métadonnées Swift/ObjC. Le correctif précédent (`setenv` dans `keyboard_init()`) était trop tardif — le constructeur `__attribute__((constructor))` de glib s'exécute au chargement de `libglib` par dyld, avant tout code Dart. Corrigé en appelant `setenv("G_SLICE", "always-malloc", 1)` dans `applicationWillFinishLaunching` de `AppDelegate.swift`, qui s'exécute avant que le moteur Dart de Flutter démarre et avant que `libaudio_input.dylib` soit ouvert via `dlopen`.
- **Plantage macOS (corruption de tas / SIGTRAP)** : Élimination d'une condition de course use-after-free où l'ancien `GfpaDspHandle` était libéré dans `registerGfpaDsp` pendant que le thread audio pouvait encore exécuter son callback d'insert. `keyboard_clear_inserts()` est désormais appelé avant toute destruction de DSP pour garantir qu'aucun pointeur périmé ne subsiste dans le snapshot d'inserts du thread audio.
- **Effets GFPA non appliqués sur macOS** : Le routage clavier → effet GFPA passait par l'API master-insert de `dart_vst_host`, qui nécessitait une synchronisation inter-bibliothèques peu fiable sur macOS. Les effets sont maintenant appliqués directement dans `keyboard_render_block()` de `libaudio_input`, alignant macOS sur l'architecture Android.

### Corrigé
- **Effet GFPA sur un clavier affectant les autres claviers** : Un second slot GF Keyboard sans effet était traité par le wah/reverb/etc. du premier clavier car tout l'audio provenait d'une seule instance FluidSynth globale avec une chaîne d'inserts globale. Corrigé en donnant à chaque slot clavier sa propre instance FluidSynth (indexée par canal MIDI) et sa propre chaîne d'inserts isolée.
- **Assertion Flutter `_elements.contains(element)` lors de la connexion de câbles** : `_JackWidgetState.build` appelait `_pulseCtrl.repeat(reverse: true)` et `_pulseCtrl.value = 0` pendant une phase de build. Ces deux appels notifient synchronement les écouteurs `AnimatedBuilder`, qui appellent `setState` en plein build et déclenchent l'assertion Flutter sur les éléments sales. Corrigé en différant à la fois le démarrage `repeat()` et la réinitialisation `stop()/value=0` via `WidgetsBinding.instance.addPostFrameCallback`.
- **Audio GF Keyboard dégradé / ne ressemble plus à une soundfont** : Deux sources distinctes de double-rendu causaient l'exécution deux fois par bloc audio du FluidSynth du slot 0. (1) `startAudio()` enregistrait `keyboardRenderBlockPtr` (wrapper rétro-compatible slot 0) tandis que `syncAudioRouting` enregistrait aussi `keyboardRenderFnForSlot(0)` — deux pointeurs différents pour le même slot, tous deux acceptés par `addMasterRender`. (2) Dans la boucle de connexion clavier→GFPA, `addMasterRender(renderPtr)` était appelé pour le slot connecté, et la boucle finale « assure tous les claviers » l'ajoutait à nouveau. Corrigé en supprimant l'enregistrement de `keyboardRenderBlockPtr` dans `startAudio()` (désormais géré entièrement par `syncAudioRouting`) et en supprimant l'appel `addMasterRender` redondant dans la boucle de connexion (la boucle finale couvre déjà tous les slots).
- **GF Keyboard complètement silencieux sur macOS après la refactorisation multi-slot** : `keyboard_init()` délègue à `keyboard_init_slot(0, …)` qui retourne `2` pour un slot nouvellement créé. `AudioEngine.init()` vérifiait `ok == 1` strictement, donc `2` était interprété comme un échec et `_isNativeKeyboardActive` restait à false — le clavier ne produisait aucun son. Corrigé en changeant la vérification en `ok != 0`.
- **Les deux soundfonts jouent simultanément / mauvaise soundfont sur le GF Keyboard / son de piano désaccordé** : Trois causes racines corrigées. (1) `keyboard_load_sf()` appelait `fluid_synth_sfload(..., reset=1)` qui réinitialise les 16 canaux MIDI aux valeurs par défaut de la soundfont chargée — corrigé en passant à `reset=0`. (2) `keyboard_program_select()` est un no-op en C si l'instance FluidSynth du slot cible n'existe pas encore. Quand `keyboard_init_slot()` appelle plus tard `_kb_create_synth()`, cette fonction rejouait toutes les soundfonts avec `reset=1`, imposant le programme 0 de la dernière soundfont sur TOUS les canaux (ex. guitare saturée au lieu du piano) — corrigé en passant aussi la boucle de `_kb_create_synth` à `reset=0`. (3) Les appels `keyboard_program_select()` antérieurs à la création du slot étant silencieusement ignorés, Dart doit les rejouer après création — corrigé en faisant retourner `2` à `keyboard_init_slot()` pour les slots nouvellement créés, et en faisant appeler `VstHostService.syncAudioRouting()` via un callback `onNewSlot` (`AudioEngine.reapplySlotPrograms`) pour rétablir les bonnes sélections de programme.

### Architecture
- **Multi-instances FluidSynth (`keyboard_synth.c`)** : Jusqu'à 4 instances FluidSynth isolées (`KbSlot`) sont désormais supportées. Le canal MIDI `n` correspond au slot `n % 4`. Les soundfonts sont chargées dans tous les slots actifs dans le même ordre, garantissant des sfIds cohérents sans suivi par-slot côté Dart.
- **Trampolines de rendu statiques** : `keyboard_render_block_0/1/2/3` fournissent des adresses C uniques par slot ; `keyboard_render_fn_for_slot(int)` retourne la bonne. `addMasterRender` de dart_vst_host reçoit désormais un pointeur unique par slot clavier.
- **API d'inserts par-slot** : `keyboard_add_insert_slot(slotIdx, fn, ud)` et `keyboard_clear_inserts_slot(slotIdx)` remplacent l'ancienne chaîne globale. Des wrappes compatibles pour le slot 0 sont conservées.
- **`AudioInputFFI` gagne l'API multi-slot** : `keyboardInitSlot`, `keyboardDestroySlot`, `keyboardRenderFnForSlot`, `keyboardAddInsertSlot`, `keyboardClearInsertsSlot`.
- **`syncAudioRouting` initialise les slots à la demande** : chaque `GrooveForgeKeyboardPlugin` dans le rack déclenche `keyboardInitSlot(midiChannel-1 % 4, 48000)`.
- **Suppression du fallback oscillateur sinusoïdal** : La branche `#else` dans `keyboard_synth.c` a été supprimée. macOS compile le chemin FluidSynth complet.
- **`VstHost` expose les accesseurs DSP bruts** : `gfpaDspInsertFn(handle)` et `gfpaDspUserdata(handle)`.
- **L'API master-insert de `dart_vst_host` n'est plus utilisée pour le clavier** : les connexions clavier → GFPA contournent désormais `dvh_add_master_insert`.

### Corrigé (précédemment)
- **Stabilité macOS (Correction du SIGTRAP)** : Résolution d'un plantage critique de corruption de mémoire (`EXC_BREAKPOINT`) en implémentant un double tampon complet pour les rendus maîtres et les inserts GFPA. Ajout d'un mécanisme de synchronisation du cycle audio (`dvh_mac_sync_audio`) qui garantit que le thread audio a "évacué" l'ancien état du rack avant que Dart ne désalloue la mémoire du plugin ou du DSP.
- **Stabilité de l'interface macOS** : Résolution du plantage persistant `EXC_BAD_ACCESS` (SIGSEGV) dans le rendu de texte Skia (`SkFont::getWidthsBounds`) et de la "RECURSION LEVEL 39" dans Dart. Implémentation d'une stratégie robuste `ClipRect` + `TextOverflow.visible` pour contourner un bug de mesure d'ellipse Skia sous macOS.
- **Isolation et Optimisation du Rendu** : Restauration de `RepaintBoundary` pour les slots de plugins afin d'isoler les cycles de peinture et mise en œuvre du cache `TextPainter` pour le piano virtuel, stabilisant l'interface lors d'activités MIDI intenses.
- **Routage du clavier macOS (FluidSynth)** : Activation du moteur FluidSynth natif sur macOS. L'audio du clavier circule désormais correctement à travers les inserts et effets maître de l'hôte VST (ex. Wah, Réverb).
- **Refonte de l'AudioEngine** : Unification de la gestion des soundfonts desktop dans `AudioEngine.dart` via un nouveau chemin FFI natif commun à Linux et macOS. 
- **Système de construction VST3 (macOS)** : Restauration de la configuration `ffiPlugin: true` dans `pubspec.yaml` et nettoyage des inclusions du SDK.
- **Corrections d'imports macOS** : Résolution des échecs de construction causés par des imports `dart:io` manquants et `WidgetsBinding` dans divers fichiers d'interface et de service.
...
### Corrigé
- **Audio natif macOS** : reconstruction de `libaudio_input.dylib` et `libdart_vst_host.dylib` pour garantir que tous les symboles sont correctement exportés. Activation du routage Master-render et Master-insert sur macOS, permettant aux instruments natifs (Theremin, Stylophone) d'utiliser les effets de descripteur GFPA (Wah, Delay, Reverb).
