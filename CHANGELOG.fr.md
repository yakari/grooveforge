...
### Corrigé
- **Générateur de batterie — bouton actif non mis à jour visuellement** : le slot UI n'était pas enveloppé dans un `ListenableBuilder`, donc appuyer sur le bouton lecture/arrêt mettait à jour le moteur mais le bouton restait dans son ancien état jusqu'à un rebuild non lié.
- **Générateur de batterie — mauvais sons / pas de percussions** : le canal MIDI de batterie était initialisé avec la banque 0 (instruments mélodiques) au lieu de la banque 128 (percussions GM). Le moteur appelle désormais `assignPatchToChannel(channel, 0, bank: 128)` lors de la création de session et du changement de soundfont.
- **Générateur de batterie — charleston décalé** : les grilles de pas dans tous les fichiers `.gfdrum` fournis avaient un caractère de trop (17 au lieu de 16 pas), décalant légèrement le charleston en avance sur la mesure suivante.
- **Générateur de batterie — sélecteur de soundfont était un sélecteur de fichier** : remplacé par un menu déroulant listant toutes les soundfonts déjà chargées dans l'application (même liste que le slot GF Keyboard).
- **Générateur de batterie — compte-à-rebours manquant un temps / chopsticks déclenchant toutes les notes simultanément** : le planificateur utilisait des positions de temps transport absolues pour les hits du compte-à-rebours. Comme le transport fire le temps 1 immédiatement et avance à la position 1.0 avant de notifier, tous les hits se retrouvaient dans le passé dès le premier tick et se déclenchaient en même temps. Corrigé avec une planification basée sur l'époque : `grooveEpoch` capture la position transport au démarrage, et chaque timestamp de mesure est calculé comme `grooveEpoch + indexMesure × duréeMesure`.
- **Générateur de batterie — formule de durée de mesure incorrecte pour les patterns non-4/4** : `barDuration` était calculée comme `timeSigNumerator` (temps), incorrect pour ex. 6/8 (donne 6 au lieu de 3 temps de noire). Formule corrigée : `numérateur × 4 ÷ dénominateur`.
