...
### Ajouté
- Une piste peut s'enregistrer directement depuis le rack — soundfont, plugin VST3, générateur de batterie — sans repasser par la pièce ni latence à compenser.
- Les répétitions et le rack cohabitent dans deux onglets : quittez un morceau pour retoucher un son et retrouvez-le toujours en lecture.
- La vue arrière du rack se referme d'une flèche ou d'un geste retour.
- Les répétitions choisissent leur micro et leur sortie, sans toucher à ceux du rack.

### Corrigé
- Un étalonnage lancé depuis les Paramètres atteint désormais un morceau déjà ouvert. Les prises enregistrées ensuite tombaient jusqu'à un quart de seconde à côté du temps alors que l'étalonnage, lui, était juste.
- Le clic ne reste plus au tempo du morceau précédent, ce qui taillait les prises à un tempo qui n'était pas le leur et les faisait rejouer trop vite.
...
- Le compteur de mesures ne passe plus sur deux lignes au-delà de la dixième, ce qui déplaçait le bouton d'enregistrement sous un doigt déjà parti vers stop.

### Architecture
- Le moteur de répétition et la sonde de latence sont rendus sur le périphérique audio du rack, sur toutes les plateformes, au lieu d'un second périphérique à eux — une seule horloge partout.
...
- Android : GrooveForge prend maintenant le focus audio, donc les autres lecteurs s'arrêtent au lieu de se mélanger aux retours — le vieux problème de l'application musicale qui démarre dès qu'on branche un jack. C'est aussi ce qui fait que les touches de transport ci-dessus arrivent jusqu'à GrooveForge : la plateforme les route en grande partie selon qui détient le focus.
- Android : brancher un DAC USB-C pouvait couper le son de toutes les applications du téléphone jusqu'au redémarrage — la touche média bloquée ci-dessus en était la cause, chaque répétition remuant le routage audio, et créer un patch audio pour un flux low-latency (MMAP) sur une sortie USB fait carrément planter le serveur audio de cet appareil (un bug d'ordre de verrous dans son compteur de dose sonore, livré en assertion fatale). Capturer les touches règle les deux, et les 4 ms du chemin rapide sont conservées ; en filet de sécurité, un flux qui meurt à répétition dans les deux secondes suivant son ouverture bascule sur le chemin mixeur plus lent au lieu de boucler indéfiniment.
- Android : un flux de sortie qui ne parvenait pas à se rouvrir restait mort jusqu'à la fin de la session, l'application muette tant qu'on ne la relançait pas. Un chien de garde réessaie désormais toutes les quelques secondes jusqu'au retour du flux.
- Android : la récupération d'un flux déconnecté fait une seule tentative, après avoir laissé le routage se stabiliser, au lieu de six tentatives espacées. Chaque tentative ouvre deux fois, en exclusif puis en partagé : réessayer transformait un changement de routage en une douzaine d'ouvertures en trois secondes, ce qui maintenait le service audio en NO_SERVICE et laissait le flux coupé bien plus longtemps que si l'on n'avait rien fait.
- Audio Harmonizer : grésillement important, surtout sur Android. Deux causes — les voix vers l'aigu étaient resynthétisées avec un recouvrement de trames insuffisant, imposant un trémolo à l'harmonie (4 dB à l'octave supérieure, 16 dB à deux octaves), et sur Android le DSP était compilé sans optimisation, manquant purement et simplement l'échéance du callback audio.
...
- Audio Harmonizer : trois ou quatre voix poussaient la sortie au-delà du niveau maximal et le périphérique audio l'écrêtait brutalement — un grésillement d'autant plus marqué que les voix étaient nombreuses. Les voix d'harmonie sont des copies transposées d'un même signal : leurs crêtes s'alignent et la somme approche la somme arithmétique des mix, soit 2,4x l'entrée avec les réglages par défaut, sans rien en aval pour atténuer. Le trajet wet est désormais plafonné à l'unité, et ajouter des voix redistribue le niveau au lieu de l'augmenter.
- Android : le flux audio tournait au tampon minimal, deux salves (4 ms sur un téléphone récent), sans jamais s'adapter. Le moindre vol de CPU coûtait un callback entier et produisait un clic. Le tampon grandit désormais d'une salve à la fois, uniquement en réponse à des sous-alimentations réellement survenues et jusqu'à un plafond de huit salves : un appareil qui ne bronche pas conserve la latence la plus basse possible.
...
### Architecture
- Le host suit désormais les notes tenues via un notifier appartenant au moteur audio, et non via celui de chaque canal. Charger un projet *remplace* les objets d'état par canal, ce qui orphelinait en silence tout listener attaché à leur création — le moteur continuait d'annoncer les notes sans que personne ne les entende, si bien que CHORD IN et Scale Lock ne faisaient rien sur un projet enregistré tout en marchant sur un rack neuf.
- Le câble `chordOut → chordIn` existait déjà pour le mode Jam et Xen ; l'harmonizer s'y enregistre comme un suiveur de plus, sans rien ajouter au système de patch.
- Android : le périphérique de capture micro journalise désormais la fréquence d'échantillonnage réellement accordée. Tout ce qui suit lit le tampon micro en supposant 48 kHz : un périphérique tournant à une autre fréquence le vide ou le remplit à un rythme constant — audible comme un son qui se coupe à intervalle régulier, sans rien dans les journaux pour l'expliquer.
- `gf_pitch` est un nouveau détecteur de hauteur monophonique partagé, nécessaire parce que le Scale Lock doit savoir quelle note est chantée avant de pouvoir aligner une harmonie sur une gamme — un harmonizer MIDI reçoit la note, un harmonizer audio non. Il utilise la différence normalisée à moyenne cumulée de YIN plutôt qu'une autocorrélation simple : la corrélation note une période doublée aussi bien que la vraie, et la moitié des hauteurs testées revenaient une ou deux octaves trop bas. Mesuré sur des voix de synthèse dont le fondamental est délibérément plus faible que les harmoniques, il situe chaque hauteur, du mi grave de guitare au mi5 de soprano, à moins de 3,3 cents.
- Le Scale Lock lit les mêmes `validPitchClasses` par canal que le piano virtuel et l'Harmonizer MIDI : il suit donc Xen ou le mode Jam sans plomberie séparée. La gamme suivie se choisit au câble : l'Audio Harmonizer reçoit la même prise **SCALE IN** que le clavier et le vocodeur, si bien que relier le SCALE OUT d'un module Xen désigne exactement le module qui s'applique — sans ambiguïté avec plusieurs modules Xen dans le rack. Le SCALE OUT du mode Jam se câble de la même façon. Un effet audio n'a pas de canal MIDI d'où déduire une gamme, et sans câble l'harmonizer reste chromatique : le Scale Lock ne fait alors rien. La gamme parvient au DSP sous forme de `scale_mask`, volontairement absent des paramètres du descripteur : c'est un état de l'hôte, et l'enregistrer rouvrirait un morceau verrouillé sur la gamme active au moment de la sauvegarde.
- Le mode Harmonie du vocodeur lit le micro via son propre curseur progressif, comme le module Live Input, au lieu de l'instantané « les N derniers échantillons » qu'utilisent les modes à banc de filtres. L'instantané convient à un modulateur mais pas à un vocodeur de phase : le callback de rendu réclame 96 trames quand le périphérique de capture en écrit 256, si bien que l'écrivain ne bouge pas à la plupart des appels et que les mêmes échantillons étaient relus deux ou trois fois avant un saut de 256. Mesuré sur une voix de synthèse au timing réel d'Android, cela produisait plus de bruit inharmonique que de signal (166 %, et 369 % avec un micro faible) contre 0,9 % désormais, avec un signal utile 87 fois plus fort.
- Le mode Harmonie évite aussi deux traitements que les modes à banc de filtres appliquent au micro, et un troisième à la sortie : le passe-haut partagé à ~380 Hz (adapté à un modulateur, mais qui retire le fondamental du chant), la porte de bruit échantillon par échantillon (qui met à zéro des échantillons isolés et crée donc un plat à chaque passage par zéro), et `soft_clip` (un waveshaper, ~7 % d'harmonique 3 sur une crête de 0,9, nécessaire aux modes porteurs forts mais pas à un moteur qui se plafonne déjà à l'unité). Il utilise à la place un bloqueur de continu à 8 Hz, une porte à suivi d'enveloppe et un simple écrêtage.
- Le moteur de traitement de l'harmonizer est déplacé dans `gf_harmony` — les vocodeurs de phase, la mise en réserve de latence qui permet à un vocodeur quantifié par trames de remplir des blocs audio entiers, l'étage de marge de crête et le redémarrage par voix. L'effet Audio Harmonizer et le mode Harmonie du vocodeur partagent désormais cette unique implémentation ; le test de fumée confirme que la sortie de l'effet est inchangée après le déplacement.
- Les noms de modes du vocodeur passent par `AppLocalizations` au lieu d'être des littéraux anglais dans le widget.
- `gf_harmonizer_smoke_test` fait passer de l'audio réel dans l'insert de l'harmonizer et vérifie la marge de crête, la continuité des blocs à toutes les tailles de salve qu'un appareil peut fournir, la régularité du gain sur toute la plage d'intervalles, et qu'une voix re-transposée à chaque bloc — la façon dont le mode Harmonie du vocodeur le pilote — ne hoquette pas. Lancé par `scripts/run_smoke_tests.sh`. Face à la 2.17.4, il échoue à trois de ses quatre contrôles — aucun n'était visible depuis la suite Dart.
- `gfpa_dsp.h` déclarait `bool` sans inclure `<stdbool.h>` : aucune unité de compilation C ne pouvait l'inclure. Corrigé ; le nouveau test de fumée est le premier consommateur C.
...
### Architecture
- CI Android : le workflow de release récupère désormais les sous-modules git. Sans eux, les sources de FluidSynth sont absentes et la configuration CMake du build NDK échoue.
- CI Android : Flutter figé en 3.41.6 pour correspondre au pubspec.lock versionné ; le pin 3.41.2 précédent résolvait d'autres paquets fournis par le SDK et cassait les builds verrouillés.
- Android : les APK par ABI utilisent désormais le schéma de version code de F-Droid (versionCode * 10 + indice d'ABI) au lieu de celui de Flutter. L'APK universel conserve son version code simple.

## [2.17.3] - 2026-09-03

### Modifié
- Guide : sections ajoutées pour la Thérémine et le Stylophone, le Générateur de batterie, les loopers et les MIDI FX.
- Guide : la barre d'onglets ne montre plus que les icônes. Les cinq libellés n'ont jamais tenu dans la fenêtre et étaient tronqués à trois lettres ; chaque icône porte désormais son libellé en infobulle.

### Corrigé
- Plus aucun son lorsqu'une autre application occupait le flux audio exclusif à faible latence. Le flux échouait à démarrer sans nouvelle tentative, et plus rien n'était rendu jusqu'au redémarrage de l'application ; il bascule désormais sur un flux partagé.
- Une gamme microtonale restaurée au démarrage calait bien les notes sur la gamme mais les jouait en tempérament égal, l'accordage n'étant jamais envoyé au synthé créé après la restauration du rack.
- Guide : le titre débordait de la fenêtre en français, et dans toute langue dont la traduction est plus longue que l'anglais.
- Les en-têtes de rack affichaient le dernier segment de l'identifiant pour chaque module .gfpd — « harmonizer », « reverb », « eq », « wah » — au lieu du nom déclaré par le descripteur.
...
- Panneau Xen : les onglets de famille sont des libellés soulignés plutôt que des boutons, les rangées de familles et de toniques se replient au lieu de sortir de l'écran sur téléphone, et Expérimental et Mes gammes passent en fin de liste.
...
### Ajouté
- **Xen — un nouveau module d'effets MIDI pour le verrouillage de gamme et le réaccordage microtonal.** C'est la synthèse de deux choses que GrooveForge faisait déjà séparément : le Jam Mode verrouille un clavier sur une gamme, Microtone atteint les hauteurs entre les touches. Xen fait les deux depuis un seul module, sans avoir besoin d'un second clavier pour lui dire quelle est l'harmonie. Tenez une note, appuyez sur une gamme : la note tenue devient la tonique, le clavier se verrouille sur la gamme construite dessus, et — pour les gammes qui ne sont pas tempérées — les touches qu'elle utilise sont réaccordées pour sonner comme la tradition l'entend et non comme un piano l'approxime. Contrairement à la détection par note de basse du Jam Mode, qui redérive la fondamentale en continu depuis un clavier maître, une tonique fixée ne bouge plus jusqu'au geste suivant : c'est la différence entre accompagner l'harmonie d'un autre et choisir la sienne.
- **43 gammes réparties en sept familles**, chacune accompagnée de la provenance de ses valeurs d'accordage. Modes, pentatoniques et blues occidentaux ; maqamat arabes et makam Rast turc dans le système à 53 commas d'Arel-Ezgi-Uzdilek ; ragas de l'Inde du Nord en intonation juste ; gammes chinoises, japonaises et coréennes ; l'échelle quasi-juste du chanter de la cornemuse écossaise ; slendro et pelog javanais ; et les tempéraments historiques (juste, pythagoricien, mésotonique 1/4 de comma, Werckmeister III). Là où une tradition n'a pas d'accordage de référence unique — le gamelan avant tout — le module le dit, plutôt que de présenter une moyenne comme un standard.
- **Le réaccordage est polyphonique.** Le plugin Microtone existant plie la hauteur, et le pitch-bend MIDI est par canal : une seule note microtonale peut donc sonner à la fois. Xen réaccorde chaque touche indépendamment via le support du MIDI Tuning Standard de FluidSynth, si bien qu'un accord entier peut tenir un tempérament non égal : un accord de maqam Rast sort avec une tierce abaissée d'un quart de ton sur une fondamentale et une quinte intactes. Disponible sur bureau et Android ; les instruments VST3 n'ont pas d'équivalent et sont ignorés.
- **Snap et tune sont deux étages distincts**, activables séparément et dotés chacun de son jack en face arrière. Vouloir la disposition de touches d'un maqam sans ses quarts de ton — pour rester juste avec un groupe à hauteur fixe — ou son intonation sur tout le clavier chromatique sont deux vrais choix musicaux, pas des cas dégénérés. Deux câbles font que le patch montre lui-même lequel est en jeu : `scaleOut` violet pour le verrou, `tuningOut` turquoise pour l'accordage.
- **Une rangée de pads peut le piloter.** Le nouveau mode de mapping CC `direct` associe un pad matériel à une gamme précise : une grille de pads devient une grille de boutons de rappel — pad 1 pour le maqam Rast, pad 2 pour le raga Yaman, pad 3 pour le slendro — au lieu d'un seul bouton qui avance dans 43 gammes. `snap` et `tune` sont aussi mappables séparément : couper le verrouillage en plein solo tout en gardant l'intonation du maqam est un geste de jeu, pas un réglage. Les gammes sont stockées par nom, de sorte qu'un mapping continue de désigner la même gamme quand le catalogue s'agrandit.
- **Le piano virtuel montre le désaccordage.** Les touches réaccordées reçoivent une bande turquoise et leur valeur en cents ; les touches hors gamme sont grisées comme sous le Jam Mode. Sur la tonique Do, le maqam Rast utilise exactement les touches blanches de do majeur — sans les marqueurs, ses quarts de ton sont audibles mais invisibles, l'état le plus difficile à suivre quand on joue.
- **Gammes expérimentales et éditeur de gammes.** Le catalogue gagne une famille d'accordages construits — 24-EDO (le système de quarts de ton des guitares refrettées de King Gizzard), 19-EDO, 31-EDO, Bohlen-Pierce, harmoniques 8-16, et un amas de quarts de ton — et un onglet « Mes gammes » où le musicien construit les siennes. La liste de degrés de l'éditeur n'a pas de longueur fixe, ce qui rend les deux formes atteignables : un sous-ensemble qui tient dans une octave sur la disposition clavier familière, ou une division qui étale une octave sur dix-neuf ou vingt-quatre touches réelles.
- **Muter un degré, distinct de le retirer.** Un degré muté sonne toujours et garde son propre accordage ; c'est seulement le snap qui cesse de le considérer comme destination. C'est ainsi qu'on ajoute un quart de ton à une gamme blues comme couleur sur laquelle s'appuyer en solo, sans que toutes les notes voisines y soient tirées. Retirer le degré, à l'inverse, laisse la touche chromatique et hors gamme.
- **Import et export de gammes.** Scala `.scl` dans les deux sens — le format d'échange du monde microtonal, qui ouvre l'archive publique de plusieurs milliers d'accordages — plus le JSON propre à GrooveForge pour un aller-retour fidèle. Toutes les hauteurs survivent exactement à un aller-retour `.scl`, mais pas la disposition clavier : le format ne sait pas dire à quelle touche appartient un degré, donc une gamme qui entasse plusieurs degrés dans un demi-ton revient en disposition linéaire, au son identique et au doigté différent.
- Les gammes custom sont stockées à deux endroits volontairement : dans la bibliothèque de l'app pour être disponibles dans tous les projets, et intégralement dans le `.gf` qui en utilise une, pour qu'un projet ouvert sur une machine qui n'a jamais vu cette gamme la joue correctement.

### Architecture
- **Modèle de gammes et d'accordage** (`GFScale`, `GFScaleDegree`, `GFScaleLibrary` dans `grooveforge_plugin_api`). Une gamme est une liste de degrés portant chacun deux informations indépendantes : sur quelle touche du clavier le degré se pose, et de combien de cents cette touche doit sonner à côté du tempérament égal. C'est cette séparation qui permet à une seule structure de décrire une gamme majeure ordinaire, un maqam arabe à quarts de ton, un raga indien en intonation juste et un tempérament historique complet. Le slendro utilise une disposition linéaire distincte, un pas de 240 cents ne correspondant à aucune touche de piano.
- **Réaccordage microtonal dans le synthé natif** — `keyboard_set_key_tuning()` / `keyboard_clear_tuning()` dans `keyboard_synth.c`, avec `gf_native_set_key_tuning()` / `gf_native_clear_tuning()` comme équivalents Android et les liaisons FFI des deux côtés. Une table de 128 écarts en cents, appliquée par canal MIDI, qui réaccorde aussi les voix déjà en train de sonner — un changement de gamme sous un accord tenu s'entend donc immédiatement. `AudioEngine.applyChannelTuning()` / `reapplyChannelTunings()` l'installent, en ignorant les canaux qui ne peuvent pas être réaccordés ainsi (VST3, et les oscillateurs natifs stylophone/thérémine, dont la hauteur est calculée en Hz avant d'atteindre un synthé). Les accordages sont réinstallés après chaque chargement de SoundFont, car en charger un avec le flag de reset de FluidSynth efface les affectations d'accordage par canal — sans cela, un maqam repasserait silencieusement en tempérament égal sans rien pour l'expliquer.
- `RackState` conserve un `GFXenPlugin` vivant par slot Xen et ne publie au moteur que le *résultat* : classes de hauteur autorisées, écarts en cents et tonique, par canal. Toute la connaissance musicale reste dans `GFScale`. Le Jam Mode avait fait l'inverse — ses tables d'intervalles ont été recopiées dans `audio_engine.dart` sous un commentaire demandant aux lecteurs futurs de maintenir les deux copies à la main — et c'est cette duplication qu'on évite ici. Le verrou de Xen est prioritaire sur le verrouillage par canal et sur le Jam Mode, sa fondamentale venant d'un geste explicite du musicien plutôt que d'un détecteur d'accords.
- Xen mémorise, par canal MIDI, la hauteur sur laquelle chaque touche jouée a été snappée, et relâche les notes à cette hauteur plutôt qu'à celle reçue. Le snapping n'est pas une transposition fixe — la même touche pointe ailleurs après un changement de fondamentale — donc une note tenue pendant un changement de gamme serait sinon relâchée sur une hauteur que rien ne joue, et resterait bloquée indéfiniment.
- `SlotParamTarget` gagne une charge utile `directValue` pour `CcParamMode.direct`, stockée comme identifiant stable et non comme index, et conservée lors d'un échange de slots. Les mappings directs sont anti-rebondis par `(slot, paramètre, valeur)` et non par `(slot, paramètre)` : deux pads liés à deux gammes sont deux intentions, et le second appui ne doit pas être avalé comme un rebond du premier.
- `gf_tuning_smoke_test` (`native_audio/`, hôte uniquement) rend de la vraie sortie FluidSynth et mesure la hauteur obtenue, en vérifiant à la fois qu'une table à −50 cents décale une touche d'un quart de ton exactement, et qu'une table de maqam Rast appliquée à un seul canal abaisse la tierce d'un accord tenu sans toucher à sa fondamentale ni à sa quinte. Sans lui, une compilation de FluidSynth dépourvue du support d'accordage rejouerait toutes les gammes microtonales en tempérament égal, sans erreur nulle part. `scripts/run_smoke_tests.sh` le compile et le lance aux côtés du smoke test du phase vocoder, dans `native_audio/build-smoke/` pour ne pas toucher à l'arbre de build habituel. Volontairement absent de la CI : ces tests se lient au FluidSynth installé sur la machine hôte, donc un échec peut signaler une vraie régression comme une différence d'environnement, et leur sortie mérite d'être lue plutôt que réduite à un code de retour.
- Rien n'a eu besoin de changer pour écrire des fichiers `.scl`. `GFScaleDegree` stocke déjà séparément la touche d'un degré et son écart : leur somme est exactement la distance à la tonique que le format demande. Le seul piège que l'écriture doit respecter est qu'une valeur avec point est en cents alors qu'une valeur sans point est un rapport de fréquences — écrire `1200` au lieu de `1200.000000` serait relu comme le rapport 1200/1, dix octaves plus haut.
- Le contrôle « un degré reste près de sa touche » du catalogue ne s'applique plus qu'aux gammes appartenant à une tradition. C'est la bonne règle là où il y a une notation à respecter, et la mauvaise pour les familles expérimentale et custom, où décrocher une touche d'un demi-ton entier de sa hauteur nominale est tout l'intérêt.
- `GFScaleLibrary` gagne un registre à l'exécution : gammes livrées, enregistrées et importées se résolvent toutes par le même point de recherche — un projet enregistré nomme une gamme par son identifiant et n'a pas à savoir d'où elle vient.
- Les smoke tests hors ligne ne sont plus compilés dans les builds de release desktop. Ce sont des outils de développement construits par nom depuis `scripts/run_smoke_tests.sh`, mais les cibles CMake faisaient partie du build par défaut : chaque release Windows, Linux et macOS payait leur compilation — et une erreur dans une cible réservée aux tests pouvait casser un build de plateforme, ce qui est précisément arrivé : `gf_tuning_smoke_test` demandait `m` à l'éditeur de liens, une vraie bibliothèque sous Unix mais un `m.lib` inexistant sous MSVC. `EXCLUDE_FROM_ALL` rend désormais vrai le commentaire « host-side only » au-dessus de ces cibles, et libm n'est demandée que sous Unix.

### Corrigé
- **Une note jouée après une pause arrivait sans attaque, uniquement à l'écoute via un dongle DAC USB-C.** Ces dongles coupent l'alimentation de leur ampli casque après environ une seconde de silence numérique et la rétablissent avec une rampe de démarrage anti-pop, et le bus AAudio partagé leur envoyait exactement cela : le flux reste ouvert toute la session et se contente d'écrire des zéros entre les notes. Le haut-parleur du téléphone n'a pas cette logique, le backend Linux non plus, d'où un symptôme qui n'apparaissait que via un dongle. Le bus mélange désormais à sa sortie un signal de maintien inaudible, pour que le flux ne soit jamais numériquement silencieux. Le niveau comme la forme d'onde ont été établis par la mesure sur un Galaxy Z Fold 6 avec un dongle USB-C vers jack, et non devinés : du bruit large bande réveille le dongle à -40 dBFS et échoue à -90 dBFS, son déclencheur est donc un seuil de niveau et pas un détecteur de zéro ; et un signal carré à Nyquist fonctionne à -50 dBFS, il lit donc les échantillons entrants et pas la sortie analogique. Placer toute l'énergie à Nyquist (24 kHz à 48 kHz) est ce qui donne les deux propriétés à la fois : 40 dB de marge au-dessus de ce seuil, de quoi tenir face à des dongles qui s'endorment plus vite, tout en restant hors de la bande audible. Ajouté uniquement dans le buffer de sortie d'AAudio, en aval de tout le reste : les enregistrements de l'Audio Looper sont inchangés au bit près.
- **La vue de patch se repeignait en boucle tant qu'elle restait ouverte.** Ses câbles data virtuels sont redérivés dans une nouvelle liste à chaque build, et l'enregistrement de câble n'avait pas d'égalité de valeur : le `shouldRepaint` du painter renvoyait donc toujours vrai ; et comme peindre programme un `setState` pour repositionner les pastilles de déconnexion, chaque repaint provoquait le suivant. Présent depuis le premier tracé des câbles accord/gamme du Jam Mode.
- **Plantages possibles sur les appareils Android à pages mémoire de 16 Ko.** `androidx.datastore`, tiré par `shared_preferences`, embarque un `libdatastore_shared_counter.so` précompilé avec le NDK r20 — toujours vrai en datastore 1.3.0-alpha10, aucune montée de version n'y change donc rien. Toutes les autres bibliothèques natives de l'app sont compilées en NDK r27 ou r28 et alignées sur 16 Ko ; celle-ci était la seule que la console Play signalait. Elle n'est chargée que par `MultiProcessDataStore`, or `shared_preferences` utilise la fabrique mono-processus : la bibliothèque est maintenant exclue du bundle au lieu d'être livrée sans jamais servir.
- **Le contenu passait sous la barre de navigation gestuelle à partir d'Android 15.** Les applis ciblant le SDK 35+ sont affichées edge-to-edge par défaut et l'app ne gérait pas les insets de fenêtre : le bas de chaque écran défilant se retrouvait sous la barre, dont le dernier élément de la liste des préférences. Le corps de chaque écran est désormais enveloppé dans un `SafeArea`, haut exclu puisque l'AppBar couvre déjà la barre d'état. Aucun changement visible sur Android 14 et antérieurs, où ces insets valent zéro.

## [2.16.0] - 2026-08-31

### Corrigé
- **Latence de sortie Android (~0,5 s) sur le clavier à l'écran et les contrôleurs MIDI**. Le bus AAudio partagé demandait une *capacité* de buffer de 4096 frames, ne fixait jamais la *taille* du buffer, et laissait le flux étiqueté comme audio MEDIA. Les trois coûtent de la latence : la demande de capacité initialise un buffer de 2048 frames (~43 ms) sur le chemin MMAP et un AudioTrack de 4096 frames (~85 ms) sur le chemin legacy — où elle désactive aussi le dimensionnement par bursts qu'AAudio applique tout seul ; l'absence de taille de buffer empêche le flux de descendre au minimum que l'appareil peut tenir ; et l'usage MEDIA fait passer la sortie par la chaîne de post-traitement média du constructeur (Dolby Atmos et équivalents), un étage d'effets à latence fixe qui ajoute 100 à 200 ms sur beaucoup de téléphones, invisible pour les mesures de latence d'AAudio. Le bus laisse maintenant la capacité au système, cale le buffer sur deux bursts (~4–10 ms) et se déclare en audio GAME/MUSIC pour contourner la chaîne de post-traitement. L'envoi des notes passait déjà en FFI direct depuis la 2.12.6 : le délai restant venait entièrement du flux de sortie, pas du bus d'effets. Mesuré sur un Galaxy Z Fold 6 (Android 16) : le buffer de sortie passe de 2048 frames (43 ms) à 192 frames (4,0 ms), avec LOW_LATENCY et EXCLUSIVE (MMAP) tous deux accordés.
...
- La compilation Windows ne casse plus sur MSVC 14.51 (Visual Studio 18), vers lequel le runner `windows-latest` de GitHub a basculé et qui transforme la dépréciation de longue date de `<experimental/coroutine>` en erreur bloquante STL1011. `windows/CMakeLists.txt` définit désormais `_SILENCE_EXPERIMENTAL_COROUTINE_DEPRECATION_WARNINGS` avant les règles de plugins générées, afin que la définition atteigne toutes les cibles. Deux plugins incluent encore cet en-tête et aucun ne dépend de nous : `permission_handler_windows`, et `universal_ble`, tiré transitivement par `flutter_midi_command` pour le MIDI BLE et donc impossible à mettre à jour isolément. Solution d'attente : Microsoft compte supprimer ces en-têtes, après quoi les plugins devront passer au `<coroutine>` C++20.
...
### Ajouté
- **Microtone MIDI FX**. Nouveau module MIDI FX qui fusionne les notes tenues simultanément en une seule hauteur microtonale via un modèle de ré-attaque : chaque changement de l'ensemble des touches tenues coupe la voix précédente et attaque une nouvelle voix sur la médiane du cluster, avec le pitch-bend déjà appliqué — le synthé joue toujours une note propre pré-accordée sur le microton, jamais un glissando sur une voix tenue. Presser C + E joue un D légèrement détoné ; presser C + C# joue une note un quart-de-ton plus haut. Presser une nouvelle touche ré-attaque immédiatement sur la nouvelle médiane ; retirer un doigt d'un cluster ré-attaque sur le cluster réduit (idéal pour les motifs rapides en retirant les doigts un à un), mais relâcher tout le cluster quasi simultanément est détecté comme un arrêt et produit un seul Note-Off sans note supplémentaire indésirable. Un réglage **Attack Delay** (0–80 ms) choisit comment la première note d'un cluster est déclenchée : à 0 ms elle se déclenche instantanément (latence nulle, brève attaque chromatique avant que le second doigt n'arrive), et à des valeurs plus élevées l'attaque est différée pour regrouper les touches pressées simultanément et jouer une seule note microtonale propre. Une frappe relâchée avant le délai se déclenche quand même (puis se relâche) pour ne jamais perdre de note. Rend n'importe quel clavier MIDI standard expressif pour la musique microtonale et xénharmonique, sans matériel spécial. Réglages : attack delay, mode de cluster (Outer Average / Mean of All), plage de pitch-bend (±2 / ±12 / ±24 demi-tons — doit correspondre au synthé en aval) et mode de vélocité (Average / Last Note). Inspiré de mTonal et de l'approche pitch-bend MTS-ESP utilisée par ODDSound et le KOMA Monoplex.
...
### Ajouté
- **Effet Harmoniseur Audio** (Linux, macOS, Android). Nouvel effet audio intégré qui ajoute jusqu'à quatre voix d'harmonie transposées à n'importe quelle source audio, avec intervalle, mix par voix et dry/wet master. Câblez-le après un clavier, un stylophone, un theremin ou un plugin VST3 et dessinez une harmonie.
- **Looper audio — lecture synchronisée au tempo** (Linux, macOS, Android). Les boucles enregistrées à un tempo suivent maintenant le transport sans changer de hauteur : une boucle de batterie enregistrée à 120 BPM jouée à 140 BPM reste en accord. Les clips joués à leur tempo d'enregistrement restent inchangés.
- **Mode NATURAL du vocoder réécrit**. La porteuse « Naturelle » ne sonne plus hachée sur les notes en dessous de la hauteur du chanteur. Elle boucle maintenant un nombre entier de périodes vocales capturées au micro, donc la couture est continue par construction et il n'y a plus de modulation sous-harmonique.

### Corrigé
- **Le Vocoder se mettait à jouer du piano après avoir changé son canal MIDI**. Déplacer un slot Vocoder vers un nouveau canal MIDI laissait le moteur audio pointé sur le canal précédent, donc les notes sur le nouveau canal tombaient vers le synthé piano par défaut. La carte de canal est maintenant mise à jour immédiatement à chaque changement.
- **Le looper audio enregistrait du silence quand un vocoder y était câblé** (Linux, macOS). Enregistrer un vocoder dans un slot de looper capturait du silence ou du bruit corrompu parce que le DSP vocoder était avancé deux fois par bloc audio. Le vocoder met maintenant en cache sa sortie par bloc pour que la sortie principale et le looper voient le même audio.

## [2.12.6] - 2026-04-12

### Corrigé
- **Android — les notes de clavier étaient laggy et les notes d'accord jouaient désynchronisées**. Les notes seules sur le clavier à l'écran ou MIDI matériel avaient un délai perceptible, et les accords se décalaient de plusieurs millisecondes par note. La dispatch de notes emprunte maintenant un chemin direct vers le synthé, faisant chuter la latence par note de ~9 ms à ~0,3 ms et atteignant la parité avec Linux/macOS.

## [2.12.5] - 2026-04-12

### Ajouté
- **Looper audio — le stylophone et le vocoder peuvent maintenant être enregistrés sur Android**. Les deux instruments atteignent enfin le bus audio partagé, donc les câbler vers un slot de looper ou un effet GFPA enregistre leur audio comme n'importe quelle autre source.
- **Looper audio — l'audio des plugins VST3 peut maintenant être enregistré sur macOS**, alignant le comportement sur Linux. Auparavant, un VST3 câblé vers un slot de looper ne capturait que du silence.
- **Looper audio — avertissement mémoire**. Le looper teinte maintenant son étiquette de mémoire par clip en ambre puis en rouge à mesure que l'utilisation totale du pool approche 256 Mo, et affiche un toast quand le plafond souple est franchi. L'enregistrement continue au-delà du plafond pour qu'une performance live ne soit jamais tronquée silencieusement.

### Corrigé
- **Looper audio — arrêter une boucle pouvait effacer l'enregistrement avant qu'il soit sauvegardé**. Appuyer sur Stop (au lieu de Pause pendant la lecture) effaçait silencieusement la longueur enregistrée, et l'autosave suivant supprimait ensuite le fichier audio sur disque comme orphelin. Les boucles arrêtées survivent maintenant à l'autosave et se rechargent correctement.
- **Looper audio — « Enregistrer sous » ne persistait pas l'audio enregistré**. Sauvegarder un projet sous un nouveau nom écrivait le fichier projet mais oubliait d'exporter les fichiers audio du looper, donc rouvrir ce projet jouait du silence. « Enregistrer sous » exporte maintenant l'audio à côté du projet.
- **Looper audio — l'utilisation mémoire affichait toujours 0 Ko sur Android**.

## [2.12.4] - 2026-04-12

### Ajouté
- **Looper audio — sélecteur de source**. Le slot du looper audio dispose maintenant d'un sélecteur « Source » compact au-dessus de la forme d'onde, pour choisir n'importe quelle source audio du rack sans avoir à basculer sur le panneau arrière et câbler manuellement.

### Corrigé
- **Build web cassé par une modification récente du looper**.

## [2.12.3] - 2026-04-12

### Ajouté
- **Looper audio — persistance WAV sur Android**. Les boucles enregistrées survivent maintenant à un redémarrage de l'application sur Android, alignant le comportement sur le desktop. Les clips sont sauvegardés à côté du fichier projet et restaurés à la réouverture.

### Corrigé
- **Looper audio — enregistrer un clavier sur Android produisait des clips silencieux**. Plusieurs chemins de code sautaient la carte de routage clavier lors de la reconstruction du graphe audio, donc les clips enregistraient la bonne durée mais ne capturaient que des zéros. Tous les chemins de reconstruction passent maintenant la carte de routage correctement.
- **Looper audio — l'aperçu de forme d'onde Android restait vide après enregistrement, et l'autosave plantait en exportant les fichiers WAV**.
- **Looper audio — couverture localisation française/anglaise complète**. Les placeholders de la forme d'onde, toutes les infobulles de la barre de transport et toutes les étiquettes de statut passent maintenant par le système de localisation.

## [2.12.2] - 2026-04-12

### Ajouté
- **Looper audio — Android n'enregistre plus que les instruments câblés**, alignant le comportement sur Linux. Avant, chaque clip actif enregistrait tout le mixage master, donc les overdubs recapturaient leur propre lecture et il n'y avait aucun moyen d'isoler un seul instrument. Les claviers, générateurs de batterie et le theremin sont supportés comme sources câblées sur Android ; le stylophone et le vocoder passent encore par le mixage master.

## [2.12.1] - 2026-04-12

### Ajouté
- **Looper audio — bouton d'assignation CC**. Le slot du looper audio expose maintenant son propre bouton d'assignation CC dans l'en-tête, avec « Bouton Loop » (cycle armer/enregistrer/jouer/overdub) et « Stop » comme cibles assignables.

### Corrigé
- **Les modules du rack restaient muets jusqu'à ce qu'on fasse défiler jusqu'à eux**. L'initialisation audio-critique — enregistrement de session, allocation DSP native, câblage du bus audio Android — ne s'exécutait avant que lorsqu'un slot devenait visible à l'écran, donc une action CC « démarrer » visant un module hors écran lançait le transport mais produisait du silence. Tous les types de modules s'initialisent maintenant dès le chargement du projet, indépendamment de la position de défilement : générateurs de batterie, looper MIDI, looper audio, effets audio GFPA, stylophone et theremin.

## [2.12.0] - 2026-04-11

### Ajouté
- **Looper Audio (PCM)**. Nouveau module de rack qui enregistre et boucle de l'audio stéréo en temps réel avec synchro mesure, overdub et reverse. Câblez n'importe quel instrument (ou plugin VST3) vers ses entrées audio, appuyez sur le bouton loop, et la boucle suit le transport. Seules les sources câblées sont enregistrées — pas de métronome ni de bleed indésirable. Le workflow à un seul bouton reflète le looper MIDI : idle → armer → enregistrer → jouer → overdub → jouer.
- **Enregistrement synchronisé à la mesure**. En mode synchro mesure, l'enregistrement démarre exactement au prochain temps fort et la boucle est complétée jusqu'au prochain temps fort à l'arrêt, pour que chaque boucle soit alignée sur des mesures complètes. Un mode libre est disponible par clip pour un enregistrement non quantifié.
- **Aperçu de forme d'onde** sur la carte du looper avec curseur de lecture et indicateur de progression d'enregistrement.
- **Les boucles sont sauvegardées avec le projet**. L'audio est enregistré en fichiers WAV annexes à côté du projet et restauré à la réouverture.
- **Assignation CC** pour le bouton loop et l'action stop.
- **Le vocoder peut maintenant être enregistré dans le looper ou traité par un effet GFPA** sur Linux/macOS.
- **Plus de contamination percussion sur les captures de clavier**. La percussion (canal 10) tourne maintenant sur son propre slot de synthé dédié, donc les clics de métronome ne contaminent plus le looper quand un clavier y est câblé.

## [2.11.0] - 2026-04-08

### Ajouté
- **Routage vers un périphérique audio USB sur Android**. La sortie des synthés (claviers, theremin, stylophone, vocoder) peut maintenant être dirigée vers un périphérique audio USB spécifique depuis le sélecteur de sortie. Quand le périphérique sélectionné est débranché en cours de session, la lecture bascule vers le périphérique par défaut et une notification s'affiche. Nécessite Android 9 ou supérieur.
- **Écran de débogage audio USB**. Nouvel écran dans les Préférences listant tous les périphériques audio que le système Android rapporte — utile pour diagnostiquer les problèmes de routage multi-USB sur du matériel inhabituel.
- **Macro d'échange de canaux**. Un CC matériel peut maintenant être assigné à l'échange des canaux MIDI de deux slots instruments, optionnellement avec toute leur chaîne de signal (câbles audio, références Jam Mode, mappings CC). Pratique pour alterner en direct entre un son de clavier et un son de pad sur le même contrôleur.
- **Mappings CC par projet**. Les assignations CC sont maintenant sauvegardées à l'intérieur du fichier projet au lieu d'être globales. Changer de projet change toute la configuration de la surface de contrôle.
- **Plusieurs assignations par numéro de CC**. Un seul potard peut maintenant contrôler plusieurs cibles à la fois, avec six types de cibles disponibles (CC GM, action système, paramètre de slot, échange de canaux, transport, global).
- **Packaging Linux**. Le workflow de release produit désormais des paquets `.rpm` (Fedora), Arch/Manjaro et Flatpak en plus des `.deb` et `.zip` existants.

### Corrigé
- **Le paquet Debian n'avait pas ses dépendances runtime**. Le `.deb` déclare maintenant correctement ses dépendances avec les noms de paquets Debian/Ubuntu.
- **Les mappings CC étaient perdus au redémarrage de l'application**. Les mappings n'étaient en fait jamais relus depuis le fichier projet au démarrage.
- **Les préférences CC plantaient à la sélection de la catégorie Looper**.
- **Android — le premier slot de clavier GF était muet après démarrage** à cause d'une course créant des instances de synthés en double pendant le chargement du projet.

## [2.10.0] - 2026-04-06

### Ajouté
- **Backend audio PipeWire / JACK sur Linux**. GrooveForge s'enregistre maintenant comme un vrai client JACK avec des ports stéréo nommés, se connecte automatiquement à la sortie système, et fonctionne à la fois sur PipeWire et JACK2 natif. La latence audio cible passe d'environ 50 ms à moins de 10 ms.
- **Routage audio inter-applications sur Linux**. L'audio de GrooveForge peut maintenant être câblé vers et depuis d'autres applications audio Linux (Ardour, Bitwig, Carla) à travers le graphe JACK standard, visible dans Helvum, qjackctl ou le patchbay de Carla.
- **Looper — curseur de volume par piste**. Chaque piste du looper a maintenant son propre contrôle de volume aux côtés de mute et solo.
- **Looper — assignation CC**. Le slot du looper a maintenant son propre bouton d'assignation CC avec mode apprentissage, supportant enregistrer/jouer, jouer/pause, stop, effacer et overdub comme cibles assignables. Les messages CC matériels câblés au looper sont routés sans câblage supplémentaire dans le rack.

### Corrigé
- **Clavier GF — l'instrument du second clavier n'était pas restauré au redémarrage**. Le second clavier revenait toujours en piano même quand le projet sauvegardé disait autre chose.
- **Le build Web / WASM était cassé** par le correctif du second clavier.

## [2.9.0] - 2026-03-25

### Ajouté
- **Générateur de pistes de batterie**. Nouveau module de rack qui génère des grooves de batterie synchronisés au transport avec humanisation (jitter de vélocité, microtiming, notes fantômes), swing, et fills et breaks structurés. Partage le même transport que le looper et le métronome.
- **Format de pattern `.gfdrum`**. Un format YAML lisible pour écrire des patterns de batterie avec grilles de pas, configuration de vélocité et timing par instrument, et support à la fois des patterns en boucle et des séquences mesure par mesure. Les utilisateurs peuvent écrire leurs propres patterns et les charger dans le générateur.
- **Dix patterns inclus** : Classic Rock, Jazz Swing, Bossa Nova, Tight Funk, Irish Jig, Breton An Dro, Reel Écossais, Batucada, Marche Militaire et Jazz Half-Time Shuffle. Les dix ont été retravaillés à partir de recherches musicologiques pour l'authenticité : kick hémiolique de Bonham sur Classic Rock, ride spang-a-lang authentique sur Jazz Swing, variation clave sur 4 mesures sur Bossa Nova, cinq variations sur Tight Funk (Funky Drummer, Cissy Strut, Sly Stone, kick syncopé, version légère), modèle de frappe bodhrán sur Irish Jig, double grosse caisse signature sur Breton An Dro, hi-hat en croches et backbeat sur Reel Écossais, ensemble 5 voix avec agogô, chamada tamborim et appel repique sur Batucada.
- **Générateur de batterie — synchronisation automatique de la signature rythmique**. Charger un pattern de batterie alors que le transport est arrêté règle maintenant la signature rythmique pour correspondre au pattern (6/8 pour Bossa Nova, 4/4 pour Rock, etc.) pour que le métronome et le compteur de mesures restent synchronisés avec le style.
- **Générateur de batterie — sauvegarde automatique des paramètres**. Swing, humanisation, soundfont, pattern, compte-à-rebours et fréquence des fills sont maintenant sauvegardés avec le projet.
- **Site de documentation GitHub Pages**. Page marketing et manuel statique publiés à la racine du site du projet, incluant un guide de prise en main, des présentations de modules, la documentation des MIDI FX et VST3, et des guides pour les formats `.gfdrum` et `.gfpd`. Miroir français sur `/fr/` avec un sélecteur de langue EN/FR complet.
- **Démo Flutter WASM** déplacée sous `/demo/`.

### Corrigé
- **Web / WASM — les notes de batterie sonnaient comme un piano**. Le pont audio navigateur ignorait la banque de batterie et utilisait toujours les samples mélodiques, donc le générateur de batterie et la percussion GM jouaient tous les deux des notes de piano. La batterie utilise maintenant une vraie TR-808 dans le navigateur.
