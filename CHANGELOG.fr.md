...
### Architecture
- **Modèle de gammes et d'accordage pour le futur module Xen** (`GFScale`, `GFScaleDegree`, `GFScaleLibrary` dans `grooveforge_plugin_api`). Une gamme est stockée comme une liste de degrés portant chacun deux informations indépendantes : sur quelle touche du clavier le degré se pose, et de combien de cents cette touche doit sonner à côté du tempérament égal. C'est cette séparation qui permet à une seule structure de décrire une gamme majeure ordinaire, un maqam arabe à quarts de ton, un raga indien en intonation juste et un tempérament historique complet. Le catalogue livre 43 gammes réparties en sept familles — modes occidentaux, maqam (arabes et turcs à 53 commas), ragas, Chine/Japon/Corée, cornemuse écossaise, gamelan, et les tempéraments (juste, pythagoricien, mésotonique 1/4 de comma, Werckmeister III) — chacune accompagnée de sa provenance, parce que plusieurs de ces traditions n'ont pas d'accordage de référence unique et qu'une table moyennée doit le dire. Le slendro utilise une disposition linéaire distincte, un pas de 240 cents ne correspondant à aucune touche de piano.
- **Réaccordage microtonal dans le synthé natif** — `keyboard_set_key_tuning()` / `keyboard_clear_tuning()` dans `keyboard_synth.c`, avec `gf_native_set_key_tuning()` / `gf_native_clear_tuning()` comme équivalents Android et les liaisons FFI des deux côtés. Ces fonctions pilotent le support du MIDI Tuning Standard de FluidSynth : une table de 128 écarts en cents, appliquée par canal MIDI, qui réaccorde aussi les voix déjà en train de sonner. C'est ce qui rendra le module Xen polyphonique là où le plugin Microtone existant ne peut pas l'être — le pitch-bend est par canal et déplace ensemble toutes les notes tenues, alors qu'une table d'accordage déplace chaque touche séparément : un accord peut donc tenir trois intervalles d'intonations différentes en même temps.
- `gf_tuning_smoke_test` (`native_audio/`, hôte uniquement) rend de la vraie sortie FluidSynth et mesure la hauteur obtenue, en vérifiant à la fois qu'une table à −50 cents décale bien une touche d'un quart de ton exactement, et qu'une table de maqam Rast appliquée à un seul canal abaisse la tierce d'un accord tenu sans toucher à sa fondamentale ni à sa quinte. Sans lui, une compilation de FluidSynth dépourvue du support d'accordage rejouerait silencieusement toutes les gammes microtonales en tempérament égal, sans erreur nulle part.
- **Le plugin Xen lui-même** (`GFXenPlugin`) — le geste signature du module et ses deux étages de traitement. Tenir une note et appuyer sur une gamme verrouille la note la plus grave comme tonique ; contrairement à la détection par note de basse du Jam Mode, qui redérive la fondamentale en continu depuis un clavier maître, une tonique verrouillée ne bouge plus jusqu'au geste suivant — c'est la différence entre accompagner l'harmonie d'un autre et choisir la sienne. L'étage snap (quantifier les notes entrantes sur les touches de la gamme) et l'étage tune (appliquer l'intonation propre de la gamme) s'activent séparément, parce que vouloir la disposition de touches d'un maqam sans ses quarts de ton, ou son intonation sans le verrouillage, sont deux vrais choix musicaux et non des cas dégénérés. Le plugin reçoit sa source de notes tenues et sa destination d'accordage sous forme de callbacks injectés : la logique du geste est donc testable sans moteur audio.
...
- `AudioEngine.applyChannelTuning()` / `reapplyChannelTunings()` installent la table de cents d'une gamme sur un canal MIDI, en aiguillant vers le chemin FFI desktop ou Android. Les canaux qui ne peuvent pas être réaccordés ainsi (instruments VST3, et les oscillateurs natifs stylophone/thérémine, dont la hauteur est calculée en Hz avant d'atteindre un synthé) sont explicitement ignorés. Les accordages sont réinstallés après chaque chargement de SoundFont, car en charger un avec le flag de reset de FluidSynth efface les affectations d'accordage par canal — sans cela, un maqam repasserait silencieusement en tempérament égal sans rien pour l'expliquer.
- `scripts/run_smoke_tests.sh` compile et lance les smoke tests natifs hors ligne (`gf_tuning_smoke_test`, `gf_pv_smoke_test`) dans `native_audio/build-smoke/`, sans toucher à l'arbre de build habituel. Volontairement absent de la CI : ces tests se lient au FluidSynth installé sur la machine hôte, donc un échec peut signaler une vraie régression comme une différence d'environnement, et leur sortie mérite d'être lue plutôt que réduite à un code de retour.
- **Le câblage face arrière de Xen** — nouveaux ports `tuningOut` / `tuningIn` (turquoise), volontairement distincts des `scaleOut` / `scaleIn` violets du Jam Mode. Verrouiller un clavier sur une gamme et réaccorder les touches que cette gamme utilise sont deux choix musicaux indépendants — la disposition d'un maqam en restant juste avec un groupe à hauteur fixe, ou son intonation sur tout le clavier chromatique — donc deux câbles, et le patch montre lui-même lequel est en jeu. `GFpaPluginInstance` gagne une liste `tuningTargetSlotIds` correspondante ; les projets enregistrés avant l'existence de Xen la lisent comme vide. Débrancher ou supprimer un slot réaccordé remet son canal en tempérament égal, plutôt que de laisser un synthé désaccordé sans rien à l'écran pour l'expliquer.
- `RackState` conserve un `GFXenPlugin` vivant par slot Xen et ne publie au moteur que le *résultat* : un ensemble de classes de hauteur autorisées par canal (`AudioEngine.xenScaleLocks`) plus une table d'accordage par canal réaccordé. Toute la connaissance musicale reste dans `GFScale`. Le Jam Mode avait fait l'inverse — ses tables d'intervalles ont été recopiées dans `audio_engine.dart` sous un commentaire demandant aux lecteurs futurs de maintenir les deux copies à la main — et c'est cette duplication qu'on évite ici. Le verrou de Xen est prioritaire sur le verrouillage par canal et sur le Jam Mode, sa fondamentale étant fixée par un geste explicite du musicien plutôt que déduite par un détecteur d'accords.
- **`CcParamMode.direct`** — un mapping CC qui règle un paramètre sur une valeur fixe portée par le mapping lui-même (`SlotParamTarget.directValue`). C'est ce qui transforme une rangée de pads matériels en rangée de boutons de rappel : pad 1 pour le maqam Rast, pad 2 pour le raga Yaman, pad 3 pour le slendro. Le mode `cycle` ne pouvait pas le faire : il ne fait qu'avancer dans une liste, donc atteindre la dixième gamme demandait neuf appuis sans possibilité de revenir. Les valeurs sont stockées comme identifiants stables et non comme index, si bien qu'un mapping continue de désigner la même gamme quand le catalogue s'agrandit, et elles survivent à un échange de slots. Les mappings directs sont anti-rebondis par `(slot, paramètre, valeur)` et non par `(slot, paramètre)` : deux pads liés à deux gammes sont deux intentions, et le second appui n'est jamais avalé comme un rebond du premier.
- Xen expose `scale`, `root`, `next_scale`, `snap`, `tune` et `latch_root` au sélecteur de cibles CC. `snap` et `tune` activent les deux étages du module indépendamment : couper le verrouillage en plein solo tout en gardant l'intonation du maqam est un geste de jeu, pas un réglage.
- **Le panneau Xen dans le rack** — une UI de slot dans le style RC-20, disposée dans l'ordre où on s'en sert : un LCD ambre qui nomme la tonique, la gamme et la provenance de ses valeurs d'accordage ; une grille des 43 gammes avec onglets par famille ; une bande chromatique de tonique avec un bouton FIXER ; et les deux listes de cibles, coloriées comme leurs jacks de face arrière (violet pour le verrou de gamme, turquoise pour l'accordage) pour que l'avant et l'arrière du module racontent la même chose. Sélectionné via `com.grooveforge.xen` dans l'aiguillage des slots et ajouté depuis la feuille des modules.
- Les boutons de gamme portent un souligné turquoise quand la gamme réaccorde réellement les touches, et le LCD un badge « ¢ ». La distinction est facile à manquer et elle compte : le maqam Hijaz et le maqam Rast sont côte à côte dans la même grille et un seul des deux est microtonal.
- La ligne sous la grille nomme la note qui va devenir la tonique pendant qu'elle est encore tenue, au lieu de laisser le musicien découvrir après coup quelle note le geste a retenue. Elle n'écoute que les notes actives des canaux sources, pour qu'un accord de passage ne reconstruise pas quarante boutons de gamme.
- Le panneau est vérifié en largeurs téléphone (360 dp), tablette (700 dp) et bureau (1280 dp) : les onglets de famille et la bande de tonique défilent horizontalement, la grille de gammes se replie, et les deux listes de cibles s'empilent sous 480 dp.
