...
### Corrigé
- **Latence des accords Android — appels audio déplacés hors du thread UI** : `playNote`, `stopNote`, `controlChange` et `pitchBend` dans `FlutterMidiProPlugin.kt` s'exécutaient auparavant sur le thread principal Android (le même que le Choreographer vsync). Un rendu de frame démarrant juste avant l'arrivée d'une note d'accord pouvait retarder celle-ci de 10 à 20 ms. Tous les appels JNI audio en temps réel s'exécutent désormais sur un exécuteur mono-thread dédié à priorité maximale (`GrooveForge-Audio`), complètement découplé du cycle de rendu UI. `MethodChannel.Result.success(null)` est retourné à Dart avant l'appel JNI afin de débloquer immédiatement la boucle d'événements Dart.
- **Saturation due au gain par défaut sur Android** : le gain maître FluidSynth par défaut est abaissé de `5.0` à `3.0` (valeur déjà utilisée sur Linux). L'ancienne valeur de `5.0` était trop élevée sur les haut-parleurs Android et provoquait un écrêtage audible.
- **Gain Android non persisté entre les sessions** : `applyFluidSynthGain()` est maintenant appelé après chaque création d'instance FluidSynth (`loadSoundfont`, `createKeyboardSlotSynth`). Auparavant, le listener de gain se déclenchait une seule fois lors de `_restoreState` avant qu'aucune instance de synthé n'existe, si bien que les nouvelles instances héritaient silencieusement de la valeur par défaut interne de FluidSynth.

### Architecture
- **Chemin MIDI sans allocation par note** (plus de pause GC lors de la frappe d'accords) :
  - `AudioGraph.hasMidiOutTo()` : nouvelle méthode sans allocation pour tester une connexion MIDI OUT ; remplace le motif `connectionsFrom().any()` qui allouait une `List` intermédiaire par slot ciblé.
  - `RackState.applyMidiFxForChannel()` sort immédiatement si aucun MIDI FX n'est enregistré (`_midiFxInstances.isEmpty`), évitant toutes les itérations et allocations de `Set` dans le cas courant.
  - Getter `RackState.hasMidiFxPlugins` : permet au code appelant de court-circuiter entièrement le pipeline MIDI FX (y compris la construction de `TimestampedMidiEvent` et de `List`) quand le rack ne contient pas de slots FX.
  - Cache de routage par canal dans `_RackScreenState` (`_routingCache`) : slots VST3, slots GFK MIDI-only et slots GFK synth pré-classifiés par canal MIDI. Reconstruit une seule fois lors des changements structurels (slot ajouté/supprimé, câble tracé/supprimé) via des listeners sur `RackState` et `AudioGraph`. Élimine les trois scans `whereType<>().where().toList()` exécutés précédemment à chaque événement MIDI dans `_routeMidiToVst3Plugins`.
  - Cache `_looperTargets` : associe chaque slot instrument aux slots looper accessibles via câble MIDI OUT. `_feedMidiToLoopers` effectue désormais une simple recherche dans la `Map` — plus d'appel `connectionsFrom()` ni de scan `RackState.plugins.any()` par événement.
  - Cache `_looperPlaybackTargets` : associe chaque slot looper à ses cibles aval. `_handleLooperPlayback` distribue désormais les événements sans parcourir les connexions ni les plugins.
  - `_cachedTransport` : `GFTransportContext` calculé une seule fois par changement de transport au lieu d'une fois par événement MIDI.
  - Toutes les références de services (`_rackState`, `_engine`, `_audioGraph`, `_vstSvc`, `_looperEngine`, `_transportEngine`) stockées en champs dans `_RackScreenState`, éliminant les traversées de l'arbre de widgets via `context.read<T>()` depuis le callback MIDI.
