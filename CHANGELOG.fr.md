...
### Corrigé
- **Arpégiateur : ajout des divisions 1/64, 1/16T et 1/32T** — le sélecteur de Division propose désormais 9 options (1/4, 1/8, 1/16, 1/32, 1/64, 1/4T, 1/8T, 1/16T, 1/32T). À 120 BPM, 1/64 = 31 ms/pas et 1/32T = 42 ms/pas, rendant pratiques les arpèges sur une large plage d'octaves.
- **Arpégiateur : le piano ne défile plus vers les notes de l'arp** — le défilement automatique de `didUpdateWidget` dans le piano virtuel ne suit désormais que les notes que l'utilisateur presse physiquement (`_pointerNote`). Les pas autonomes de l'arpégiateur traversant plusieurs octaves ne volent plus le viewport en pleine performance. Le comportement de défilement vers la note racine pour l'Harmoniseur et le Chord Expand est préservé.
- **Arpégiateur : touches visuellement bloquées après un glissando** — faire glisser rapidement la souris sur le piano virtuel laissait des touches en surbrillance dans `activeNotes` de façon permanente. Deux causes racines : (1) `ArpeggiateNode._handleUserNoteOn` appelait `_fireStep`, qui ajoutait la hauteur au sentinel `_arpNoteOns` ; ce sentinel n'était jamais consommé lors d'un déclenchement inline (hors du chemin tick-merge), donc un retour de glissando identifiait à tort le note-on utilisateur comme un événement généré par l'arp, sautait `_handleUserNoteOn` et empêchait le relâchement d'émettre un note-off. (2) `_onNotePressed` passait tous les événements de `_applyMidiChain` à `engine.playNote` sans filtrer par type, y compris les gate note-offs injectés par tick() dans le même lot, ajoutant des hauteurs obsolètes à `activeNotes`. Correctifs : effacer le sentinel `_arpNoteOns` immédiatement après l'appel inline à `_fireStep` dans `_handleUserNoteOn` ; filtrer `_onNotePressed` et `_onNoteReleased` pour n'appeler `playNote`/`stopNote` qu'avec les types d'événements correspondants.

### Ajouté
- **Plugin MIDI FX Transposeur** (`com.grooveforge.transposer`) dans le menu "Ajouter un plugin" : décale toutes les notes MIDI entrantes de ±24 demi-tons. Le viewport du piano virtuel ne défile pas vers la hauteur transposée en sortie — la logique de défilement ne suit que les touches physiquement pressées (`_pointerNote`), qui restent à la hauteur d'origine.
- **Plugin MIDI FX Courbe de vélocité** (`com.grooveforge.velocity_curve`) dans le menu "Ajouter un plugin" : remappage des vélocités de note-on selon trois modes de courbe. **Power** (défaut) : v_out = (v_in/127)^exp × 127 avec exposant entre 0,25 et 4,0 (réponse douce → linéaire → réponse dure via un seul potentiomètre Amount). **Sigmoid** : courbe en S logistique centrée à la vélocité 64 ; Amount plus élevé = transition plus abrupte. **Fixed** : remplace toutes les vélocités par une valeur constante (1–127). Les note-offs et événements non-notes passent toujours sans modification.
- **Plugin MIDI FX Gate** (`com.grooveforge.gate`) dans le menu "Ajouter un plugin" : filtre les notes qui sortent d'une fenêtre de vélocité ou d'une plage de hauteur configurables. Quatre paramètres : Vel Min / Vel Max (0–127) et Note Min / Note Max (0–127). Les note-ons supprimés sont mémorisés afin que le note-off correspondant soit également supprimé, évitant les notes bloquées si les paramètres changent pendant qu'une note est tenue. Utile pour les zones de split, le rejet des notes fantômes et le routage par fenêtre de vélocité.
- **`VelocityCurveNode`** nœud MIDI intégré (`type: velocity_curve`) : implémentation 100 % Dart des trois algorithmes de courbe (power, sigmoid, fixed). Chemin rapide au centre linéaire (amount ≈ 0,5 en mode power) qui retourne la liste d'événements entrante sans modification.
- **Filtre de plage de hauteur pour `GateNode`** : le `GateNode` existant est étendu avec les paramètres `maxVelocity`, `minPitch` et `maxPitch` en plus du `minVelocity` existant. Chemin rapide qui sort immédiatement si les quatre paramètres sont à leurs valeurs par défaut (gate entièrement ouvert). Rétrocompatible : les fichiers `.gfpd` existants qui ne lient que `minVelocity` continuent de fonctionner — les trois nouveaux paramètres restent à leurs valeurs par défaut.

### Ajouté
- **Plugin MIDI FX Arpégiateur** (`com.grooveforge.arpeggiator`) dans le menu "Ajouter un plugin" : arpège les notes maintenues en une séquence rythmique. Les note-ons entrants sont supprimés et remplacés par des notes individuelles temporisées de façon autonome. Paramètres : Motif (Montant / Descendant / Aller-Retour Montant / Aller-Retour Descendant / Dans l'Ordre / Aléatoire), Division (1/4, 1/8, 1/16, 1/32, 1/4T, 1/8T), Gate (10–100 % de la durée du pas), Octaves (1–3). Joue pendant la performance live indépendamment de l'état du transport.
- **Nœud intégré `ArpeggiateNode`** (`type: arpeggiate`) : horodatage par horloge murale (`DateTime.now().microsecondsSinceEpoch`) pour un jeu indépendant du transport. Utilise des ensembles `_arpNoteOns` / `_arpNoteOffs` par canal pour distinguer les événements générés par l'arp des événements utilisateur dans la boucle de rétroaction `tick()` / `processMidi()`. Le premier pas démarre immédiatement à l'appui sur la touche, sans latence perceptible.
- **`RackState._midiFxTicker`** : `Timer.periodic` de 10 ms qui pilote les ticks de l'arpégiateur (et de tout futur MIDI FX temporel) sur tous les canaux d'instruments actifs, même en l'absence d'événements utilisateur — permet des arpèges soutenus lors du maintien d'un accord.
