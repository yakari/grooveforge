...
### Architecture
- `gf_harmonizer_smoke_test` renders real audio through the harmonizer insert and checks headroom, block continuity at every burst size a device might hand over, and gain flatness across the whole interval range. Run by `scripts/run_smoke_tests.sh`. Against 2.17.4 it fails three of its four checks — the Dart suite could not see any of them.
