...
### Fixed
- **Crash (`malloc(): unsorted double linked list corrupted`) when switching to rear rack view while an effect is ringing (Linux/macOS)**: use-after-free on the ALSA/CoreAudio audio thread. `_destroyGfpaDspForSlot` on desktop called `destroyGfpaDsp` without first removing the handle from the master-insert chain or waiting for in-flight callbacks to retire, so the audio thread's snapshotted raw pointer pointed at freed memory. Fixed by adding `dvh_remove_master_insert_by_handle(host, dspHandle)` to both backends: it searches all source chains, removes the matching entry, releases `pluginsMtx`, then spin-waits (up to 500 ms) for `callbackSeq` to advance at least once — guaranteeing any in-flight snapshot has retired before the DSP is freed. Added `std::atomic<uint64_t> callbackSeq` to `AudioState` (both ALSA and macOS backends), incremented at the end of each audio block after all DSP processing.
- **GFPA effect chains broken on Linux/macOS — only the first effect worked**: `dvh_add_master_insert` replaced the existing insert for a source instead of appending. Changed `masterInserts` from `unordered_map<DvhRenderFn, pair<GfpaInsertFn,void*>>` to `unordered_map<DvhRenderFn, vector<pair<GfpaInsertFn,void*>>>` in both backends. `dvh_add_master_insert` now push-backs with deduplication. The audio callback processes each insert in series (source → insert[0] → insert[1] → … → master mix). The desktop Dart routing rebuilder now uses the same DFS traversal (`_addChainInsertsDesktop`) as Android, replacing the single-hop `from==null && to==null` handling.
- **GFPA effects not applied to Theremin or Stylophone on Linux/macOS**: the desktop `syncAudioRouting` never registered Theremin/Stylophone as `masterRender` contributors nor wired GFPA effects into their chains. Fixed by adding a DFS pass over `GFpaPluginInstance` sources with `pluginId == 'com.grooveforge.theremin'` / `'com.grooveforge.stylophone'`: when at least one downstream GFPA connection exists, the instrument is added as a masterRender, capture mode is enabled, and all reachable GFPA effects are registered via `_addChainInsertsDesktop`.
- **Theremin sounds saturated when added to the rack (Linux/macOS)**: `syncAudioRouting` called `clearMasterInserts` but never `clearMasterRenders`, so a `thereminRenderBlockPtr` added to `masterRenders` in a previous routing state persisted indefinitely. When the Theremin later had no cables, it was rendered twice: once through its own ALSA device (capture mode OFF) and once through the dart_vst_host master mix (stale `masterRenders` entry), doubling the signal → saturation. Fixed by calling `dvh_clear_master_renders` at the start of every routing rebuild and re-adding all active sources from scratch.
- **Theremin connected to WAH (shared with Keyboard) received no effects (Linux/macOS)**: the global-dedup fix that prevented saturation was too aggressive — it also blocked the Theremin from joining the Keyboard's WAH → Reverb chain. Replaced global dedup rejection with **fan-in merging**: `masterInserts` is now a `vector<InsertChain>` (each chain has `vector<DvhRenderFn> sources` + `vector<pair<GfpaInsertFn,void*>> effects`). When `dvh_add_master_insert` is called for a source whose DSP is already in an existing chain, the source is merged into that chain's sources instead of being rejected. The audio callback mixes all sources first (fan-in), then runs the effect chain once on the combined signal, so KB2 + Theremin both go through WAH → Reverb with each DSP called exactly once per block.

### Fixed
- **Theremin produces no sound and second GF Keyboard silenced after adding Theremin (Android)**: `OBOE_BUS_SLOT_THEREMIN` was defined as 5, which collides with the FluidSynth sfId (also 5) assigned to the second keyboard's dedicated synth created by `createKeyboardSlotSynth`. `oboe_stream_add_source` treats the slot ID as a unique key: when the Theremin registered at slot 5, it got "already registered" warning (no-op); when the keyboard registered at sfId=5, it displaced or was confused with the Theremin slot. Fixed by raising all instrument bus slots to the 100-range (`kBusSlotTheremin=100`, `kBusSlotStylophone=101`, `kBusSlotVocoder=102`), mirrored in C (`OBOE_BUS_SLOT_THEREMIN=100`, etc.) and in Dart constants in `GfpaAndroidBindings`. `kMaxBusSlot` in `gfpa_audio_android.cpp` updated to 102 accordingly.

### Fixed
- **SIGSEGV in `FreeverbEffect::process` / `CombFilter::process` when switching to rear (patch) view (Android, persistent)**: `unregisterGfpaDsp` on Android had an inline early-return path (`if (Platform.isAndroid) { gfpaDspDestroy(handle); return; }`) that called `gfpaDspDestroy` directly, bypassing `_destroyGfpaDspForSlot` entirely and thus skipping the `gfpaAndroidRemoveInsert` drain-wait. The previous fixes to `_destroyGfpaDspForSlot` therefore never ran on this path. Fixed by removing the Android special-case and routing `unregisterGfpaDsp` through `_destroyGfpaDspForSlot` on all platforms.
- **SIGSEGV in `FreeverbEffect::process` / `CombFilter::process` when switching to rear (patch) view while a reverb tail is resonating (Android)**: `gfpa_android_remove_insert` held `g_chainsMtx` across its drain spin-wait. The audio callback acquires the same mutex inside `gfpa_android_apply_chain_for_sf` to take a chain snapshot — so the callback blocked waiting for the mutex, `g_callbackDoneSeq` never advanced, the 50 ms drain timeout fired, `gfpa_dsp_destroy` freed the DSP, and then the callback finally acquired the mutex, snapshotted the now-freed handle, and crashed. Fixed by releasing `g_chainsMtx` immediately after the chain mutation (before the drain wait) so the audio callback is never blocked. Timeout also increased from 50 ms to 500 ms to handle DSP-stall edge cases (observed 98 ms stalls in IsochronousClockModel).
- **SIGSEGV in `DelayEffect::process` when removing any GFPA slot from the rack (Android)**: `_destroyGfpaDspForSlot` called `gfpaDspDestroy` without first removing the handle from the insert chain, leaving a dangling pointer that the AAudio callback thread dereferenced on the next audio buffer. The existing `gfpaAndroidRemoveInsert` spin-wait drain was never exercised on this path. Fixed by always calling `gfpaAndroidRemoveInsert` (which drains the audio thread) immediately before `gfpaDspDestroy` in `_destroyGfpaDspForSlot`.
- **GFPA effect chains broken — only the first effect in a chain worked (Android)**: `_syncAudioRoutingAndroid` only followed direct keyboard→effect connections and stopped there, so a WAH→Reverb chain registered the WAH but never the Reverb. Replaced the single-hop lookup with a depth-first traversal (`_addChainInserts`) that follows every audio cable downstream from a source, registering all reachable GFPA effects in traversal order into the correct bus slot. Chaining e.g. Keyboard→WAH→Reverb now applies both effects in series.
- **Theremin and Stylophone not routed through GFPA effects in chained configurations (Android)**: the routing rebuild only considered `GrooveForgeKeyboardPlugin` sources, skipping `GFpaPluginInstance` slots with `pluginId == 'com.grooveforge.theremin'` (bus slot 5) and `'com.grooveforge.stylophone'` (bus slot 6). Extended `_syncAudioRoutingAndroid` to start a DFS chain traversal from Theremin and Stylophone sources as well.
- **"ValueNotifier used after being disposed" crash when switching to rear rack view**: in `GFpaDescriptorSlotUI._initPlugin()`, the call `_paramNotifier.addListener(_onParamChanged)` at line 92 lacked a `mounted` guard. If the widget was disposed while awaiting `_plugin.initialize()`, `_paramNotifier` was already disposed and the listener registration crashed. Added `if (!mounted) return;` before the listener call.

### Added
- **Rack state debug log on view switch**: switching between front (slot) and back (patch cable) view now emits a `debugPrint` block listing all rack modules (id, display name, type) and all active audio connections (from → to), making it easier to diagnose routing bugs from logcat output.

### Fixed
- **SIGSEGV crash when adding/removing GFPA effect modules at runtime (Android)**: `gfpa_android_remove_insert` removed the DSP handle from the insert chain and returned immediately, but the AAudio callback thread may have already snapshotted the chain and was still executing `FreeverbEffect::process` (or any other DSP) with the now-freed handle — a use-after-free (null dereference at offset 0x438 into freed memory). Fixed by exposing `oboe_stream_callback_done_seq()` from `oboe_stream_android.cpp` and spinning in `gfpa_android_remove_insert` until the counter advances (guaranteeing any in-flight snapshot has retired) before returning to the caller that will free the DSP instance. Timeout: 50 ms.
- **Theremin not affected by GFPA effects (WAH, reverb, etc.) on Android**: the Theremin was outputting audio through its own miniaudio playback device and was never registered on the shared AAudio bus, so GFPA insert chains had no way to intercept its audio. Fixed by: (1) adding `theremin_bus_render(outL, outR, frames, userdata)` (AudioSourceRenderFn-compatible wrapper) and `theremin_bus_render_fn_addr()` to `audio_input.c`; (2) adding `oboeStreamAddSource` / `oboeStreamRemoveSource` bindings in `GfpaAndroidBindings`; (3) when the Theremin plugin starts on Android, enabling capture mode (miniaudio outputs silence) and registering `theremin_bus_render` on the AAudio bus at slot 5 (`OBOE_BUS_SLOT_THEREMIN`); (4) removing from the bus and disabling capture mode on dispose. The Theremin's audio now flows through `gfpa_android_apply_chain_for_sf(5, …)` in the AAudio callback, making WAH and all other GFPA effects fully functional on the Theremin.

### Fixed
- **GFPA effect still applied to all GF Keyboards instead of only the connected one (Android)**: the root cause was that two keyboard slots using the same default soundfont path shared a single FluidSynth instance (sfId=1) on Android — `audio_engine.dart` deduplicated soundfont loading by file path, so only one JNI `loadSoundfontFile` call was made, and both slots' GFPA inserts were registered in the same bus chain. Fixed by provisioning a dedicated FluidSynth instance (unique sfId) per GF Keyboard slot via `AudioEngine.createKeyboardSlotSynth`, keyed by MIDI channel instead of soundfont path. The per-slot sfId is tracked in `_channelSlotSfId` and used by `_getSfIdForChannel` / `_applyChannelInstrument` so all MIDI note events and GFPA routing use the slot-specific synth. `RackState.initAndroidKeyboardSlots` is called after each project load and on app startup to provision the slots; `_buildKeyboardSfIds` now uses `sfIdForChannel` so each keyboard plugin maps to its own bus slot.

### Fixed
- **GFPA effects (WAH, reverb, delay, EQ, compressor, chorus) had no effect on Android keyboard audio**: the bundled pre-built `libfluidsynth.so` does not support `new_fluid_audio_driver2()` with its Oboe backend (returns NULL), so it was impossible to intercept the audio path via a FluidSynth callback. Replaced the per-synth `new_fluid_audio_driver()` approach with a shared AAudio output stream (`oboe_stream_android.cpp`) that calls `fluid_synth_process()` directly on each registered FluidSynth instance, sums the outputs, and applies the GFPA insert chain via `gfpa_android_apply_chain()` before interleaving to the device buffer. WAH and all other GFPA effects now work on Android keyboard audio.
- **Data race on GFPA scratch buffers when multiple Android keyboard slots are active**: `gfpa_audio_android.cpp` previously used static global arrays for DSP ping-pong scratch space, which would be written concurrently by two audio-thread callbacks. Scratch buffers are now stack-allocated inside `gfpa_android_apply_chain()` so each call site has its own private copy.
- **GFPA effect applied to all GF Keyboards instead of only the connected one (Android)**: the insert chain was applied to the mixed output of all synths after summing, so WAH wired to keyboard A would also process keyboard B's audio. Redesigned the routing architecture so each synth is rendered into its own per-keyboard buffer (`g_synthL[s]`/`g_synthR[s]`), its per-keyboard GFPA insert chain is applied in-place via `gfpa_android_apply_chain_for_sf(sfId, …)`, and only then the result is accumulated into the master mix. The `gfpa_audio_android.cpp` insert chains are now keyed by soundfont ID (one chain per GF Keyboard slot), and `oboe_stream_android.cpp` stores the sfId alongside each synth pointer. The Dart layer (`GfpaAndroidBindings`, `VstHostService`, `RackState`) passes the soundfont ID when registering inserts so they land in the correct per-keyboard chain.
- **SVF filter self-oscillation / sustained loud tone when pressing a key on an unlinked GF Keyboard (Android)**: was a symptom of the above routing bug — WAH's Chamberlin State Variable Filter at high Q self-oscillated when processing audio it shouldn't have received. Fixed by the per-keyboard routing fix above.

### Fixed
- **Autosave crashes on Linux when a knob is moved (ENOENT on rename)**: rapid parameter changes fired concurrent `autosave` calls that all wrote to the same `.tmp` file; the first rename succeeded and every subsequent rename failed because the temp file was already gone. Fixed by debouncing `autosave` with a 500 ms `Timer` so a burst of mutations produces exactly one write once the stream of changes settles. A `_isSaving` guard in `_performAutosave` additionally prevents any overlap with a manual `saveProject` in flight.
- **GFPA effect bleeds to both GF Keyboards instead of only the connected one**: each keyboard slot now owns a dedicated FluidSynth instance (`keyboard_render_block_0` / `keyboard_render_block_1`) with a unique C function address. `dart_vst_host` keys GFPA inserts on function pointer, so an effect wired to keyboard A can no longer intercept keyboard B's audio.
- **"Sustain pedal always held" sound (reverb tail) on macOS**: FluidSynth 2.5.3 on Homebrew ignores `synth.reverb.active=0` in settings and keeps the effect unit active. Added `fluid_synth_reverb_on(synth, -1, 0)` / `fluid_synth_chorus_on(synth, -1, 0)` runtime calls immediately after `new_fluid_synth()` as a definitive disable. (Linux FluidSynth honours the settings-only path and was already clean.)
- **Second GF Keyboard significantly quieter than first**: `keyboard_set_gain` is called during audio-engine init when only slot 0 exists; slot 1 (initialised lazily by `syncAudioRouting`) would start at FluidSynth's factory default gain (0.2) instead of the app's gain (3.0). Fixed by persisting the last-set gain in `g_current_gain` and applying it inside `_create_synth` whenever a new slot is created.
- **macOS CI build**: `libaudio_input.dylib` was a pre-built binary linking to an absolute Homebrew path (`/opt/homebrew/opt/fluid-synth/lib/libfluidsynth.3.dylib`) that does not exist on end-user machines. The CI now installs `fluid-synth` and `dylibbundler`, rebuilds `libaudio_input.dylib` from source, then uses `dylibbundler` to copy all transitive FluidSynth Homebrew dependencies into the app bundle and rewrite their references to `@rpath/`.

### Added
- **macOS audio corruption (sounds like all effects active / echo / distortion)**: `dvh_mac_start_audio` pre-allocated `extBufL/R` as 256 floats but never set `config.periodSizeInFrames`, so miniaudio let CoreAudio pick its own period (typically 512 frames). The CoreAudio callback then called `keyboard_render_block` with `frameCount = 512`, causing FluidSynth to write 512 floats into a 256-float buffer — a heap buffer overflow that corrupted adjacent memory and produced garbled audio resembling echo/wah/distortion effects. Fixed by setting `config.periodSizeInFrames = s->blockSize` (256) so the callback always receives exactly the expected frame count.
- **`libdart_vst_host.dylib` rebuilt for macOS**: the pre-built macOS dylib was outdated and missing `dvh_add_master_render`, `dvh_add_master_insert`, `dvh_clear_master_inserts`, `dvh_set_processing_order`, and `dvh_clear_routes` — APIs required for keyboard routing and GFPA effects. Rebuilt from source via CMake (universal binary, arm64 + x86_64).
- **GF Keyboard on macOS via FluidSynth**: the GF Keyboard now uses the same in-process FluidSynth path on macOS as on Linux, replacing the previous `flutter_midi_pro` fallback. `keyboard_synth.c` now compiles on both `__linux__` and `__APPLE__` platforms. `keyboard_render_block()` is registered with `dart_vst_host`'s CoreAudio thread as a master-mix contributor, so MIDI playback and GFPA effects (wah, reverb, etc.) work identically on both desktop platforms.
- **G_SLICE heap-corruption fix (macOS)**: added `setenv("G_SLICE", "always-malloc", 1)` in `AppDelegate.swift applicationWillFinishLaunching` — runs before dyld loads libglib (pulled in by FluidSynth), preventing glib's slab allocator from corrupting Swift/ObjC heap metadata and causing EXC_BAD_ACCESS crashes.
