...
### Architecture
- **Scale and tuning model for the coming Xen module** (`GFScale`, `GFScaleDegree`, `GFScaleLibrary` in `grooveforge_plugin_api`). A scale is stored as a list of degrees that each carry two independent facts: which keyboard key the degree sits on, and how many cents that key should sound away from equal temperament. Separating the two is what lets one structure describe a plain major scale, an Arabic maqam with quarter-flats, an Indian raga in just intonation, and a full historical temperament. The catalogue ships 43 scales across seven families — Western modes, maqam (Arabic and Turkish 53-comma), raga, Chinese/Japanese/Korean, Highland pipe, gamelan, and the temperaments (just, Pythagorean, quarter-comma meantone, Werckmeister III) — each with a provenance note, because several of these traditions have no single authoritative tuning and an averaged table should say so. Slendro uses a separate linear key layout, since a 240-cent step corresponds to no piano key at all.
- **Microtonal retuning in the native synth** — `keyboard_set_key_tuning()` / `keyboard_clear_tuning()` in `keyboard_synth.c`, with `gf_native_set_key_tuning()` / `gf_native_clear_tuning()` as the Android counterparts and FFI bindings on both paths. These drive FluidSynth's MIDI Tuning Standard support: a 128-entry table of cent deviations, applied per MIDI channel, that retunes voices already sounding. This is why the Xen module can be polyphonic where the existing Microtone plugin cannot — pitch bend is per channel and moves every held note together, while a tuning table moves each key on its own, so a chord can hold three differently-tempered intervals at once.
- `gf_tuning_smoke_test` (`native_audio/`, host-side only) renders real FluidSynth output and measures the pitch that comes back, asserting both that a −50 cent table shifts a key by exactly a quarter-tone and that a maqam Rast table applied to one channel flattens a held chord's third while leaving its root and fifth untouched. Without it a FluidSynth build lacking tuning support would silently play every microtonal scale back as equal temperament, with no error anywhere.
- **The Xen plugin itself** (`GFXenPlugin`) — the module's signature gesture and its two processing stages. Holding a note and tapping a scale latches the lowest held note as the tonic; unlike Jam Mode's bass-note detection, which re-derives the root continuously from a master keyboard, a latched root stays put until the gesture is repeated — the difference between accompanying someone else's harmony and choosing your own. The snap stage (quantise incoming notes to the scale's keys) and the tune stage (apply the scale's own intonation) are independently switchable, because wanting a maqam's key layout without its quarter-tones, or its intonation without the lock, are both real musical choices rather than degenerate cases. The plugin takes its held-note source and its tuning destination as injected callbacks, so the gesture logic is exercisable without an audio engine.
...
- `AudioEngine.applyChannelTuning()` / `reapplyChannelTunings()` install a scale's cent table on a MIDI channel, dispatching to the desktop or Android FFI path. Channels that cannot be retuned this way (VST3 instruments, and the stylophone/theremin native oscillators, which derive pitch in Hz before reaching a synth) are skipped explicitly. Tunings are re-installed after every SoundFont load, since loading one with FluidSynth's reset flag drops channel tuning assignments — without this a maqam would silently revert to equal temperament with nothing to indicate why.
- `scripts/run_smoke_tests.sh` builds and runs the offline native smoke tests (`gf_tuning_smoke_test`, `gf_pv_smoke_test`) into `native_audio/build-smoke/`, leaving the normal build tree alone. Deliberately not wired into CI: these link against whatever FluidSynth the host has installed, so a failure can mean either a real regression or a host difference, and the output is worth reading rather than reducing to an exit code.
- **Xen's back-panel wiring** — new `tuningOut` / `tuningIn` ports (teal), kept deliberately separate from Jam Mode's purple `scaleOut` / `scaleIn`. Locking a keyboard to a scale and retuning the keys that scale uses are independent musical choices — a maqam's key layout while staying in tune with a fixed-pitch band, or its intonation across the full chromatic keyboard — so they are two cables, and the patch itself shows which is in play. `GFpaPluginInstance` gained a matching `tuningTargetSlotIds` list; projects saved before Xen existed read it as empty. Unpatching or deleting a retuned slot returns its channel to equal temperament rather than leaving a synth detuned with nothing on screen to explain why.
- `RackState` keeps one live `GFXenPlugin` per Xen slot and publishes only the *result* to the engine: a set of allowed pitch classes per channel (`AudioEngine.xenScaleLocks`) plus a tuning table per retuned channel. All the musical knowledge stays in `GFScale`. Jam Mode went the other way — its scale interval tables were copied into `audio_engine.dart` under a comment asking future readers to keep the two copies in sync by hand — and that duplication is what this avoids. Xen's lock takes priority over both the per-channel scale lock and Jam Mode, since its root is latched by an explicit player gesture rather than inferred from a chord detector.
- **`CcParamMode.direct`** — a CC mapping that sets one parameter to one fixed value carried by the mapping itself (`SlotParamTarget.directValue`). This is what turns a bank of hardware pads into a bank of recall buttons: pad 1 selects maqam Rast, pad 2 raga Yaman, pad 3 slendro. `cycle` could not do it — it only steps through a list, so reaching the tenth scale meant nine taps and no way back. Values are stored as stable ids rather than indices, so a mapping keeps pointing at the same scale when the catalogue grows, and they survive a slot swap. Direct mappings are debounced per `(slot, param, value)` rather than per `(slot, param)`, so two pads bound to different scales are two intentions and the second tap is never swallowed as a bounce of the first.
- Xen exposes `scale`, `root`, `next_scale`, `snap`, `tune` and `latch_root` to the CC target picker. `snap` and `tune` toggle the module's two stages independently: dropping the scale lock mid-solo while keeping the maqam's intonation is a performance move, not a settings change.
- **The Xen rack panel** — an RC-20 styled slot UI laid out in the order a player uses it: an amber LCD naming the tonic, the scale and where its tuning numbers come from; a family-tabbed grid of all 43 scales; a chromatic tonic strip with a LATCH button; and the two target lists, coloured to match their back-panel jacks (purple for the scale lock, teal for the retune) so the front and back of the module agree. Selected via `com.grooveforge.xen` in the rack slot dispatcher and added from the plugin sheet.
- Scale buttons carry a teal underline when the scale actually retunes keys, and the LCD a "¢" badge. The distinction is easy to get wrong and it matters: maqam Hijaz and maqam Rast sit side by side in the same grid and only one of them is microtonal.
- The line under the grid names the note that is about to become the tonic while it is still held, rather than leaving the player to discover after the fact which note the gesture picked. It listens to just the source channels' active notes, so a passing chord does not rebuild forty scale buttons.
- The panel is verified at phone (360 dp), tablet (700 dp) and desktop (1280 dp) widths: the family tabs and the tonic strip scroll horizontally, the scale grid wraps, and the two target lists stack below 480 dp.
