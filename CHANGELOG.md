...
### Fixed
- **Vocoder plays a piano sound after changing its MIDI channel** (all platforms). Changing the Vocoder GFPA plugin's MIDI channel from the rack UI moved the plugin's `midiChannel` field but never re-applied the channel mapping to the audio engine — so `AudioEngine.channels[newChannel].soundfontPath` stayed at its default (or whatever the previous owner set) instead of getting flipped to the `vocoderMode` marker. The note dispatch in [audio_engine.dart](lib/services/audio_engine.dart) `playNote` then saw a non-vocoder soundfont path on the new channel and fell through to `keyboardNoteOn(...)` → FluidSynth → and because the C-side `_slot_for_channel` collapses every odd-indexed channel into FluidSynth slot 0, the vocoder's notes ended up doubling whatever piano sound the slot-0 keyboard was playing. The bug was visible from a fresh rack: add a Vocoder, change its channel, press a key, hear piano.
  - Fix in [rack_state.dart](lib/services/rack_state.dart) `setPluginMidiChannel`: when the plugin is a `GFpaPluginInstance` (not just a `GrooveForgeKeyboardPlugin`), call `_applyGfpaPluginToEngine(plugin)` after updating `midiChannel`. For the Vocoder branch this re-runs `assignSoundfontToChannel(newChannel - 1, vocoderMode)` so the new channel is properly marked. Also clears the **old** channel's `vocoderMode` marker so the channel the vocoder *moved away from* doesn't keep silently routing future plugins' notes through the vocoder DSP.
  - Same fix unblocks the Theremin and any future GFPA plugin that needs per-channel state — the old `if (plugin is GrooveForgeKeyboardPlugin)` guard ignored every other plugin type.
  - Surfaced while testing the Audio Harmonizer in session 4: the user was trying to free up a MIDI channel for a keyboard cabled into the harmonizer, moved the Vocoder out of the way to channel 7, and discovered the vocoder had silently turned into a duplicate piano voice. The Audio Harmonizer DSP itself was working correctly the whole time.
...
- **Audio Harmonizer GFPA effect** (Linux, macOS, Android). New built-in effect that produces up to 4 pitch-shifted harmony voices from any audio input, mixed with the dry signal. Cable any source into it (GF Keyboard, Stylophone, Theremin, audio looper output, VST3 plugin) and the wet output is the input plus pitch-shifted parallel voices at the configured intervals.
  - **DSP**: new `HarmonizerEffect` C++ class in [packages/flutter_vst3/dart_vst_host/native/src/gfpa_dsp.cpp](packages/flutter_vst3/dart_vst_host/native/src/gfpa_dsp.cpp) holds 8 phase-vocoder contexts (4 voices × 2 channels, mono per channel) preallocated in the constructor — no audio-thread allocation. Each block, the input is fed through every active voice's pair of `gf_pv_context*` with the per-voice semitone offset applied via `gf_pv_set_pitch_semitones`, the resulting pitch-shifted output is summed into the dry-seeded output bus with the per-voice mix gain, and the master `dry_wet` controls the dry/wet balance. Bypass and parameter writes use the standard `std::atomic<float>` pattern shared with all other GFPA effects.
  - **Pitch shift now actually works**: `gf_pv_pitch_shift` was an API stub through sessions 1–3 — only time-stretch was live, and `gf_pv_set_pitch_semitones` was a no-op. This release wires it up properly. The implementation in [native_audio/gf_phase_vocoder.c](native_audio/gf_phase_vocoder.c) composes the existing time-stretch with an output-side resample stage: `internal_stretch = user_stretch × pitch_ratio` drives the FFT pipeline (so the audio is stretched longer than requested), then the drain stage reads the output ring at a fractional cursor advancing by `pitch_ratio` samples per output frame with linear interpolation between adjacent samples. Net effect: `output_length = input_length × user_stretch` and every frequency component is multiplied by `pitch_ratio`, exactly as classical phase-vocoder pitch shifting prescribes. The fast path (pitch_ratio == 1) bypasses the fractional drain entirely so existing time-stretch consumers (audio looper, session 2) are bit-identical and unchanged.
  - **Smoke test extended** [native_audio/gf_pv_smoke_test.c](native_audio/gf_pv_smoke_test.c): generates a pure 440 Hz sine, calls a new offline helper `gf_pv_pitch_shift_offline` to shift it +12 semitones (octave up → 880 Hz) and −12 semitones (octave down → 220 Hz), and asserts the DFT magnitude at the expected output frequency dominates the residue at 440 Hz by ≥3×. Both shifts pass with a > 500× margin on Linux. Output WAVs are written to `/tmp/gf_pv_up12.wav` / `/tmp/gf_pv_down12.wav` for auditioning.
  - **Build wiring**: no changes required — `gfpa_dsp.cpp` is already compiled into `libdart_vst_host.so` (Linux/macOS) and `libnative-lib.so` (Android), and `gf_phase_vocoder.c` is already in those same libs from session 2's audio looper integration. The harmonizer just adds a new `#include "gf_phase_vocoder.h"` and consumes the existing symbols.
  - **Dispatch**: the plugin id `com.grooveforge.audio_harmonizer` is recognised by `gfpa_dsp_create` (new branch alongside Reverb / Delay / Wah / EQ / Compressor / Chorus). A new `GfpaEffectType::Harmonizer` enum value, union member, and switch case were added to the existing `GfpaDspInstance` dispatch in [gfpa_dsp.cpp](packages/flutter_vst3/dart_vst_host/native/src/gfpa_dsp.cpp). When the harmonizer slot enters the rack, `RackState` automatically calls `registerGfpaDsp(slotId, "com.grooveforge.audio_harmonizer")` and the JACK/Oboe insert chain wires the new effect's process callback exactly like every other built-in effect.
  - **Distinct from the existing MIDI FX**: registered under `com.grooveforge.audio_harmonizer` (not `com.grooveforge.harmonizer`, which is the pre-existing MIDI FX that adds harmony notes to incoming MIDI). The two plugins coexist — one operates on MIDI before the synth, one operates on audio after it. User-facing tiles: **Audio Harmonizer** / **Harmoniseur Audio**.
  - **10 parameters**: `voice_count` (1..4, default 2), `voice{1..4}_semitones` (-24..+24 st, defaults +7 / +12 / +4 / -5 — fifth, octave, third, fourth-down), `voice{1..4}_mix` (0..1, defaults 0.7 / 0.7 / 0.5 / 0.5), master `dry_wet` (0..1, default 0.5). Pinned by [test/plugins/audio_harmonizer_test.dart](test/plugins/audio_harmonizer_test.dart).
  - **Known limitation — first-block ring-in**: each PV context has the usual ~46 ms warm-up at FFT 2048 / hop 512 (4 voices × 2 channels = 8 contexts, all warming up in parallel). During that window the wet voices contribute zero and the dry signal passes through unattenuated, so the harmonizer "fades in" over the first ~46 ms after a fresh insert or any state reset. For sustained harmony parts this is inaudible; for staccato stabs the first attack is dry-only. Tracked as session 4b in the roadmap alongside the matching looper limitation.
