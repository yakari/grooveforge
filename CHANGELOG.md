...
- **GF Keyboard completely silent on macOS after multi-slot refactor**: `keyboard_init()` delegates to `keyboard_init_slot(0, …)` which returns `2` for a newly-created slot. `AudioEngine.init()` was checking `ok == 1` strictly, so `2` was treated as failure and `_isNativeKeyboardActive` stayed false — the keyboard produced no sound. Fixed by changing the check to `ok != 0`.
