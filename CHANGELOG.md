...
- **Autotune**: an audio effect that snaps a voice onto the notes of a key and scale, from a gentle nudge to full robot. Patch Live Input into it and sing.
- Autotune controls: Strength, Retune speed, Humanize, Flex-Tune, Transpose and Mix; a scale cable from Xen or Jam Mode overrides its own key and scale.
...
### Added
- A part can record straight from the rack — soundfont, VST3 plugin, drum generator — with nothing going through the room and no latency to compensate.
- Rehearsals and the rack now sit in two tabs: leave a tune to reach for a sound and come back to it still playing.
- The rack's back panel closes with a back arrow or a back gesture.
- Rehearsals picks its own microphone and speaker, without changing what the rack uses.

### Fixed
- Calibrating from Settings now reaches a tune that is already open. Takes recorded afterwards were up to a quarter of a second off the beat against a calibration that was right.
- The click no longer stays at the previous tune's tempo, which cut takes at a tempo that was not the tune's and left them playing back too fast.
- Tempo panel: the lock notices sit under the tempo they describe rather than under the still-editable count-in, and a tempo set by a recording shows no slider.
- The bar counter no longer wraps to a second line past bar ten, which pushed the record button out from under a finger already reaching for stop.

### Architecture
- The rehearsal engine and the latency probe play on the rack's audio device, on every platform, instead of a second device of their own — one clock everywhere.
...
### Changed
- Both harmonizers: a **CHORD IN** jack. Patch a keyboard's CHORD OUT into it and the chord you play writes the voice intervals — the notes above the lowest become the voices, so C-E-G sets two voices at +4 and +7 and the shape under your hand is the shape of the harmony. It sets the controls rather than taking them over: the lanes show what the chord asked for, stay editable, and keep their values when you let the keys go.
- Audio Harmonizer: **Scale Lock**. With it on, each voice's interval bends to the nearest note of the scale the Xen module (or Jam Mode) has locked, instead of holding a fixed semitone count. A third above is four semitones over C major and three over D minor — without this the harmony is simply wrong as soon as the chord moves. Off by default; a fixed interval is what the module did before.
- Vocoder: the fourth mode is now **Harmony**, and runs the Audio Harmonizer's engine on a continuous, unfiltered feed of the microphone. Sing, hold a chord, and it comes back in your own voice — the held keys pitch-shift the live microphone, and the filter bank is bypassed because a carrier that already *is* the voice has nothing to gain from it. It replaces the loop resampler that captured one instant of the voice and replayed it at each key's pitch, which sustained a note but lost every word and inflection. Mode index 3 is unchanged, so saved projects land on the new algorithm.
...
- Android: GrooveForge now takes audio focus, so other players stop instead of mixing themselves into the monitors — the long-standing complaint of a music app starting the moment a jack is plugged in. It is also what makes the transport keys above reach GrooveForge at all: the platform routes them largely by who holds focus.
- Android: plugging in a USB-C DAC could silence every app on the phone until a reboot — the stuck media key above was the cause, since each repeat churns audio routing, and creating an audio patch for a low-latency (MMAP) stream on a USB output crashes this device's audio server outright (a lock-order bug in its sound-dose meter, shipped as a fatal assertion). Capturing the keys stops both, and the fast 4 ms path is kept; as a safety net a stream that keeps dying within two seconds of opening falls back to the slower mixer path instead of looping forever.
- Android: an output stream that failed to reopen stayed down for the rest of the session, with the app silent until it was restarted. A watchdog now retries every few seconds until the stream comes back.
- Android: recovering from a disconnected stream tries once, after waiting for the route to settle, instead of retrying six times with a backoff. Each attempt opens twice, exclusive then shared, so retrying turned a route change into a dozen opens in three seconds — which kept the audio service answering NO_SERVICE and left the stream down far longer than doing nothing would have.
- Audio Harmonizer: heavy crackling, worst on Android. Two causes — upward voices were resynthesised with too little frame overlap, stamping a tremolo on the harmony (4 dB at an octave up, 16 dB at two), and on Android the DSP was being compiled unoptimised, so it missed the audio callback's deadline outright.
...
- Audio Harmonizer: three or four voices pushed the output past full scale and the audio device hard-clipped it, which is heard as crackle and gets worse the more voices play. Harmony voices are pitch-shifted copies of one signal, so their peaks line up and the sum approaches the arithmetic sum of the mixes — 2.4x the input at the default settings, with nothing downstream attenuating. The wet path is now capped at unity, and adding voices redistributes the level instead of increasing it.
- Android: the audio stream ran at the tightest possible buffer, two bursts (4 ms on a modern phone), and never adapted. Anything that briefly stole the CPU cost a whole callback and clicked. The buffer now grows one burst at a time, only in response to underruns that actually happened and up to a ceiling of eight bursts, so a device that never glitches keeps the lowest latency it can.
...
### Architecture
- Host code now follows held notes through a notifier owned by the audio engine rather than through each channel's own. Loading a project *replaces* the per-channel state objects, which silently orphaned every listener attached to them at startup — the engine went on reporting notes and nothing heard them, so CHORD IN and Scale Lock both did nothing on any saved project while working on a fresh rack.
- The `chordOut → chordIn` cable already existed for Jam Mode and Xen; the harmonizer registers as another follower of it, so nothing new was needed in the patch system.
- Android: the mic capture device now logs the sample rate it was actually granted. Everything downstream reads the mic ring assuming 48 kHz, so a device running at another rate drains or floods it at a fixed rate — audible as sound cutting out on a regular beat, with nothing in the logs to say why.
- `gf_pitch` is a new shared monophonic pitch tracker, needed because Scale Lock has to know which note is being sung before it can snap a harmony to a scale — a MIDI harmonizer is told the note, an audio one is not. It uses YIN's cumulative mean normalised difference rather than plain autocorrelation: correlation scores a doubled period as highly as the true one, and half the test pitches came back an octave or two low. Measured against synthetic voices whose fundamental is deliberately weaker than their harmonics, it lands every pitch from a guitar's low E to a soprano's E5 within 3.3 cents.
- Scale Lock reads the same per-channel `validPitchClasses` the virtual piano and the MIDI Harmonizer already use, so it follows Xen or Jam Mode with no separate plumbing. Which scale it follows is a cable: the Audio Harmonizer gains the same **SCALE IN** jack the keyboard and vocoder have, so patching a Xen module's SCALE OUT to it names exactly which module applies — unambiguous with several Xen modules in the rack. Jam Mode's SCALE OUT patches the same way. An audio effect has no MIDI channel to infer a scale from, and with nothing patched the harmonizer stays chromatic and Scale Lock does nothing. The scale reaches the DSP as `scale_mask`, deliberately not a descriptor parameter: it is host state, and saving it would reopen a song locked to whatever scale was active when it was saved.
- The vocoder's Harmony mode reads the mic through its own advancing cursor, the way the Live Input module does, rather than the "most recent N samples" snapshot the filter-bank modes use. The snapshot is fine for a modulator but not for a phase vocoder: the render callback pulls 96 frames while the mic capture device writes 256, so the writer does not move on most calls and the same samples were re-read two or three times before a 256-frame jump. Measured on a synthetic voice at the real Android timing, that put more non-harmonic junk in the output than signal (166%, and 369% on a quiet mic) against 0.9% now, with the wanted signal 87x stronger.
- Harmony mode also skips two things the filter-bank modes do to the mic and a third they do to the output: the shared ~380 Hz high-pass (which suits a modulator but takes the fundamental out of singing), the per-sample noise gate (which zeroes individual samples and so punches a flat spot through every zero crossing), and `soft_clip` (a waveshaper, ~7% third harmonic at a peak of 0.9, needed by the loud carrier modes and not by an engine that already caps itself at unity). It uses an 8 Hz DC blocker, an envelope-following gate, and a plain clamp instead.
- The harmonizer's signal engine moved into `gf_harmony` — the phase vocoders, the latency banking that lets a frame-quantised vocoder fill whole audio blocks, the headroom stage, and the per-voice restart. Both the Audio Harmonizer effect and the vocoder's Harmony mode now run that one implementation; the smoke test confirms the effect's output is unchanged by the move.
- Vocoder mode names go through `AppLocalizations` instead of being English literals in the widget.
- `gf_harmonizer_smoke_test` renders real audio through the harmonizer insert and checks headroom, block continuity at every burst size a device might hand over, gain flatness across the whole interval range, and that a voice re-pitched every block — how the vocoder's Harmony mode drives it — does not stutter. Run by `scripts/run_smoke_tests.sh`. Against 2.17.4 it fails three of its four checks — the Dart suite could not see any of them.
- `gfpa_dsp.h` declares `bool` without including `<stdbool.h>`, so no C translation unit could include it. Fixed; the new smoke test is the first C consumer.
...
### Added
- **Xen — a new MIDI FX module for scale locking and microtonal retuning.** It is the synthesis of two things GrooveForge already did separately: Jam Mode locks a keyboard to a scale, Microtone reaches pitches between the keys. Xen does both from one module, and needs no second keyboard to tell it what the harmony is. Hold a note, tap a scale: the note you are holding becomes the tonic, the keyboard locks to the scale built on it, and — for scales that are not equal-tempered — the keys it uses are retuned to sound as the tradition intends rather than as a piano approximates them. Unlike Jam Mode's bass-note detection, which keeps re-deriving the root from a master keyboard, a latched root stays put until you repeat the gesture: the difference between accompanying someone else's harmony and choosing your own.
- **43 scales across seven families**, each with a note on where its tuning numbers come from. Western modes, pentatonics and blues; Arabic maqamat and Turkish makam Rast in the 53-comma Arel-Ezgi-Uzdilek system; North Indian ragas in just intonation; Chinese, Japanese and Korean scales; the Great Highland Bagpipe's near-just chanter scale; Javanese slendro and pelog; and the historical temperaments (just, Pythagorean, quarter-comma meantone, Werckmeister III). Where a tradition has no single authoritative tuning — gamelan above all — the module says so rather than presenting an average as a standard.
- **The retuning is polyphonic.** The existing Microtone plugin bends pitch, and MIDI pitch bend is per channel, so only one microtonal note can sound at a time. Xen retunes each key independently through FluidSynth's MIDI Tuning Standard support, so a whole chord can hold a non-equal temperament: a maqam Rast triad comes out with a quarter-flat third over an untouched root and fifth. Available on desktop and Android; VST3 instrument targets have no equivalent and are skipped.
- **Snap and tune are separate stages**, each switchable on its own and each with its own back-panel jack. Wanting a maqam's key layout without its quarter-tones — to stay in tune with a fixed-pitch band — or its intonation across the full chromatic keyboard are both real musical choices, not degenerate cases. Two cables mean the patch itself shows which is in play: purple `scaleOut` locks, teal `tuningOut` retunes.
- **A pad bank can drive it.** The new `direct` CC mapping mode binds one hardware pad to one specific scale, so a grid of pads becomes a grid of recall buttons — pad 1 for maqam Rast, pad 2 for raga Yaman, pad 3 for slendro — instead of one button stepping through 43 scales. `snap` and `tune` are separately mappable too: dropping the scale lock mid-solo while keeping the maqam's intonation is a performance move, not a settings change. Scales are stored by name, so a mapping keeps pointing at the same scale as the catalogue grows.
- **The virtual piano shows the detuning.** Retuned keys get a teal band and their cent value; out-of-scale keys grey out as they do under Jam Mode. Rooted on C, maqam Rast uses exactly the white keys of C major — without the markers its quarter-tones are audible but invisible, which is the hardest kind of state to reason about while playing.
- **Experimental scales and a scale editor.** The catalogue gains a family of constructed tunings — 24-EDO (the quarter-tone system King Gizzard's refretted guitars play in), 19-EDO, 31-EDO, Bohlen-Pierce, harmonics 8-16, and a quarter-tone cluster — and a "My Scales" tab where the player builds their own. The editor's degree list has no fixed length, which is what makes both shapes reachable: a subset that fits inside one octave on the familiar keyboard layout, or a division that spreads one octave across nineteen or twenty-four real keys.
- **Muting a degree, as distinct from removing it.** A muted degree still sounds and still carries its own tuning; the snap stage simply stops treating it as a destination. That is how a quarter-tone gets added to a blues scale as a colour to lean on during a solo, without every nearby note being dragged onto it. Removing the degree instead leaves the key chromatic and out of the scale.
- **Scale import and export.** Scala `.scl` in both directions, which is the interchange format of the microtonal world and opens the public archive of several thousand tunings; plus GrooveForge's own JSON for a faithful round trip. Every pitch survives an `.scl` round trip exactly, but the keyboard layout does not — the format has no way to say which key a degree belongs to — so a scale that crowds several degrees into one semitone comes back on the linear layout, sounding identical and fingered differently.
- Custom scales are stored twice on purpose: in the app's library so they are available in every project, and in full inside the `.gf` file that uses one, so a project opened on a machine that has never seen that scale still plays it correctly.

### Architecture
- **Scale and tuning model** (`GFScale`, `GFScaleDegree`, `GFScaleLibrary` in `grooveforge_plugin_api`). A scale is a list of degrees each carrying two independent facts: which keyboard key the degree sits on, and how many cents that key should sound away from equal temperament. Separating the two is what lets one structure describe a plain major scale, an Arabic maqam with quarter-flats, an Indian raga in just intonation, and a full historical temperament. Slendro uses a separate linear key layout, since a 240-cent step corresponds to no piano key at all.
- **Microtonal retuning in the native synth** — `keyboard_set_key_tuning()` / `keyboard_clear_tuning()` in `keyboard_synth.c`, with `gf_native_set_key_tuning()` / `gf_native_clear_tuning()` as the Android counterparts and FFI bindings on both paths. A 128-entry table of cent deviations, applied per MIDI channel, that also retunes voices already sounding — so changing scale under a held chord is audible at once. `AudioEngine.applyChannelTuning()` / `reapplyChannelTunings()` install it, skipping channels that cannot be retuned this way (VST3, and the stylophone/theremin native oscillators, which derive pitch in Hz before reaching a synth). Tunings are reinstalled after every SoundFont load, since loading one with FluidSynth's reset flag drops channel tuning assignments — without that a maqam would silently revert to equal temperament with nothing to indicate why.
- `RackState` keeps one live `GFXenPlugin` per Xen slot and publishes only the *result* to the engine: allowed pitch classes, cent deviations and the tonic, per channel. All the musical knowledge stays in `GFScale`. Jam Mode went the other way — its scale interval tables were copied into `audio_engine.dart` under a comment asking future readers to keep the two copies in sync by hand — and that duplication is what this avoids. Xen's lock takes priority over both the per-channel scale lock and Jam Mode, since its root comes from an explicit player gesture rather than a chord detector.
- Xen records which pitch each sounding key was snapped to, per MIDI channel, and releases notes at that pitch rather than at the incoming one. Snapping is not a fixed transposition — the same key maps elsewhere after a root change — so a note held across a scale change would otherwise be released at a pitch nothing is playing and hang indefinitely.
- `SlotParamTarget` gained a `directValue` payload for `CcParamMode.direct`, stored as a stable id rather than an index and carried across slot swaps. Direct mappings are debounced per `(slot, param, value)` rather than per `(slot, param)`: two pads bound to different scales are two intentions, and the second tap must not be swallowed as a bounce of the first.
- `gf_tuning_smoke_test` (`native_audio/`, host-side only) renders real FluidSynth output and measures the pitch that comes back, asserting both that a −50 cent table shifts a key by exactly a quarter-tone and that a maqam Rast table on one channel flattens a held chord's third while leaving its root and fifth untouched. Without it, a FluidSynth build lacking tuning support would play every microtonal scale back as equal temperament with no error anywhere. `scripts/run_smoke_tests.sh` builds and runs it alongside the phase-vocoder smoke test, into `native_audio/build-smoke/` so the normal build tree is left alone. Deliberately not in CI: these link against whatever FluidSynth the host has installed, so a failure can mean a real regression or a host difference, and the output is worth reading rather than reducing to an exit code.
- Nothing had to change to write `.scl` files. `GFScaleDegree` already stores a degree's key and its deviation separately, so their sum is the distance from the tonic that the format asks for. The one trap the writer must respect is that a dotted value means cents while an undotted one means a frequency ratio — writing `1200` instead of `1200.000000` would be read back as the ratio 1200/1, ten octaves up.
- The catalogue's "a degree stays near its key" check now applies only to scales that belong to a tradition. It is the right rule where there is a notation to be faithful to, and the wrong one for the experimental and custom families, where pulling a key a full semitone off its nominal pitch is the entire point.
- `GFScaleLibrary` gained a runtime registry, so shipped, saved and imported scales all resolve through one lookup — a saved project names a scale by id and must not have to know where it came from.
- The offline smoke tests are no longer compiled into desktop release builds. They are developer tools built by name from `scripts/run_smoke_tests.sh`, but the CMake targets were part of the default build, so every Windows, Linux and macOS release paid to compile them — and a mistake in a test-only target could break a platform build, which is exactly what happened: `gf_tuning_smoke_test` asked the linker for `m`, which is a real library on Unix and a nonexistent `m.lib` on MSVC. `EXCLUDE_FROM_ALL` now makes the "host-side only" comment above those targets true, and libm is requested only on Unix.

### Fixed
- **Notes played after a pause arrived with no attack, through a USB-C DAC dongle only.** Those dongles power their headphone amp down after roughly a second of digitally silent input and bring it back with a soft-start anti-pop ramp, and the shared AAudio bus was feeding them exactly that: the stream stays open for the whole session and simply writes zeros between notes. The phone's own speaker has no such logic and neither does the Linux backend, which is why the symptom only ever appeared through a dongle. The bus now mixes an inaudible keep-alive signal into its output so the stream is never digitally silent. Both the level and the waveform were established by measurement on a Galaxy Z Fold 6 with a USB-C-to-jack dongle rather than guessed: broadband noise woke the dongle at -40 dBFS and failed at -90 dBFS, so its trigger is a level threshold and not a zero-detector; and a square wave at Nyquist works at -50 dBFS, so it reads the incoming samples rather than the analogue output. Putting all of the energy at Nyquist (24 kHz at 48 kHz) is what buys both properties at once — 40 dB of margin over that threshold, enough to stand a chance on dongles that sleep more eagerly, while sitting outside the audible band. Added only to the AAudio output buffer, upstream of nothing: Audio Looper recordings are bit-identical.
- **The patch view repainted in a loop for as long as it stayed open.** Its virtual data cables are re-derived into a fresh list on every build, and the cable record had no value equality, so the painter's `shouldRepaint` always returned true; since painting schedules a `setState` to reposition the disconnect badges, each repaint caused the next. Present since Jam Mode's chord/scale cables were first drawn.
- **Possible crashes on Android devices with 16 KB memory pages.** `androidx.datastore`, pulled in by `shared_preferences`, ships a prebuilt `libdatastore_shared_counter.so` compiled with NDK r20 — still true of datastore 1.3.0-alpha10, so no version bump fixes it. Every other native library in the app is built with NDK r27 or r28 and 16 KB aligned; this was the only one Play Console flagged. It is loaded exclusively by `MultiProcessDataStore`, and `shared_preferences` uses the single-process factory, so the library is now excluded from the bundle instead of being shipped and never used.
- **Content ran underneath the gesture navigation bar on Android 15 and later.** Apps targeting SDK 35+ are laid out edge-to-edge by default and the app handled no window insets, so the bottom of each scrolling screen sat beneath the navigation pill — the last item of the preferences list among them. Every screen body is now wrapped in a `SafeArea`, top excluded since the AppBar already covers the status bar. No visible change on Android 14 and earlier, where those insets are zero.

## [2.16.0] - 2026-08-31

### Fixed
- **Android output latency (~0.5 s) on the on-screen keyboard and hardware MIDI controllers**. The shared AAudio bus asked for a 4096-frame buffer *capacity*, never set a buffer *size*, and left the stream tagged as MEDIA audio. All three cost latency: the capacity request seeds a 2048-frame (~43 ms) buffer on the MMAP path and a 4096-frame (~85 ms) AudioTrack on the legacy path — where it also suppresses AAudio's own burst-based buffer sizing; the missing buffer size means the stream never settles at the minimum the device can sustain; and MEDIA usage routes output through the vendor's media post-processing chain (Dolby Atmos and equivalents), a fixed effect stage worth a further 100–200 ms on many phones and invisible to AAudio's latency reporting. The bus now leaves capacity to the platform, pins the buffer to two bursts (~4–10 ms), and declares itself GAME/MUSIC audio so the post-processing chain is bypassed. Note dispatch was already direct-FFI since 2.12.6 — the remaining delay was entirely in the output stream, not in the effects bus. Measured on a Galaxy Z Fold 6 (Android 16): the output buffer drops from 2048 frames (43 ms) to 192 frames (4.0 ms), with LOW_LATENCY and EXCLUSIVE (MMAP) both granted.
...
- The Windows build no longer fails on MSVC 14.51 (Visual Studio 18), which GitHub's `windows-latest` runner moved to and which promotes the long-standing `<experimental/coroutine>` deprecation to hard error STL1011. `windows/CMakeLists.txt` now defines `_SILENCE_EXPERIMENTAL_COROUTINE_DEPRECATION_WARNINGS` ahead of the generated plugin rules, so it reaches every plugin target. Two plugins still include that header and neither is ours to change: `permission_handler_windows`, and `universal_ble`, which arrives transitively through `flutter_midi_command` for BLE MIDI and so cannot be upgraded on its own. This is a stopgap — Microsoft intends to remove the headers outright, after which the plugins must move to C++20 `<coroutine>`.
...
### Added
- **Microtone MIDI FX**. New MIDI FX plugin that combines simultaneously-held notes into a single microtonal pitch using a re-attack model: every change to the held set silences the previous voice and attacks a fresh one at the new cluster median, with the pitch-bend already in place — so the synth always plays a clean note pre-tuned to the microtone, never a sliding bend on a sustained voice. Pressing C + E plays a D with a slight detune; pressing C + C# plays a quarter-tone-up note. Pressing a new key re-attacks immediately at the new median; peeling one finger off a cluster re-attacks at the smaller cluster (great for fast finger-peel patterns), but releasing the whole cluster nearly together is detected as a stop and produces a single Note-Off with no unwanted extra note. An **Attack Delay** control (0–80 ms) chooses how the first note of a cluster fires: at 0 ms it fires instantly (zero latency, brief chromatic onset before the second finger lands), and at higher values it defers the attack to gather simultaneously-pressed keys and fire one clean single microtonal note. A tap released before the delay still fires (and releases) so notes are never dropped. Makes any standard MIDI keyboard expressive for microtonal and xenharmonic music without special hardware. Parameters: attack delay, cluster mode (Outer Average / Mean of All), bend range (±2 / ±12 / ±24 st — must match the downstream synth), and velocity mode (Average / Last Note). Inspired by mTonal and the MTS-ESP pitch-bend approach used by ODDSound and the KOMA Monoplex.
...
### Added
- **Audio Harmonizer effect** (Linux, macOS, Android). A new built-in audio effect that adds up to four pitch-shifted harmony voices to any audio source, with per-voice interval, mix, and master dry/wet controls. Cable it after a keyboard, stylophone, theremin, or VST3 plugin and dial in a harmony.
- **Audio looper — tempo-synced playback** (Linux, macOS, Android). Loops recorded at one tempo now follow the transport without changing pitch: a drum loop recorded at 120 BPM played back at 140 BPM stays in tune. Clips played at their recorded tempo are unaffected.
- **Vocoder NATURAL mode rewritten**. The "Natural" carrier no longer sounds choppy on notes below the singer's pitch. It now loops an integer number of voice periods captured from the mic, so the seam is continuous by construction and there is no subharmonic warble.

### Fixed
- **Vocoder started playing piano after changing its MIDI channel**. Moving a Vocoder slot to a new MIDI channel left the audio engine pointing at the previous channel, so notes on the new channel fell through to the default piano synth. The channel mapping is now updated immediately on any channel change.
- **Audio looper recorded silence when a vocoder was cabled into it** (Linux, macOS). Recording a vocoder into a looper slot captured silence or garbled audio because the vocoder DSP was advanced twice per audio block. The vocoder now caches its per-block output so both the main output and the looper see the same audio.

## [2.12.6] - 2026-04-12

### Fixed
- **Android — keyboard notes felt laggy and chord notes played out of sync**. Single notes on the on-screen or hardware-MIDI keyboard had a perceptible delay, and chords staggered by several milliseconds per note. Note dispatch now takes a direct path to the synth, dropping per-note latency from ~9 ms to ~0.3 ms and reaching parity with Linux/macOS.

## [2.12.5] - 2026-04-12

### Added
- **Audio looper — stylophone and vocoder can now be recorded on Android**. Both instruments now reach the shared audio bus, so cabling them into a looper slot or a GFPA effect records their audio like every other source.
- **Audio looper — VST3 plugin audio can now be recorded on macOS**, matching Linux behaviour. Previously a VST3 cabled into a looper slot captured silence.
- **Audio looper — memory warning**. The looper now tints its per-clip memory label amber and red as total pool usage approaches 256 MB, and shows a toast when the soft cap is crossed. Recording continues past the cap so a live performance is never silently truncated.

### Fixed
- **Audio looper — stopping a loop could erase the recording before it was saved**. Pressing Stop (as opposed to Pause during playback) silently wiped the recorded length, and the next autosave then deleted the on-disk audio file as an orphan. Stopped loops now survive autosave and reload correctly.
- **Audio looper — "Save As" did not persist recorded audio**. Saving a project under a new name wrote the project file but forgot to export the looper audio files, so reopening that project played silence. Save As now exports the audio alongside the project.
- **Audio looper — memory usage always showed 0 KB on Android**.

## [2.12.4] - 2026-04-12

### Added
- **Audio looper — source selector**. The audio looper slot now has a compact "Source" dropdown above the waveform so you can pick any audio source in the rack without flipping to the back panel to draw cables manually.

### Fixed
- **Web build broken by a recent looper change**.

## [2.12.3] - 2026-04-12

### Added
- **Audio looper — WAV persistence on Android**. Recorded loops now survive an app restart on Android, matching desktop behaviour. Clips are saved alongside the project file and restored on reopen.

### Fixed
- **Audio looper — recording a keyboard on Android produced silent clips**. Several code paths were skipping the keyboard routing map when rebuilding the audio graph, so clips recorded length but captured only zeros. All rebuild paths now pass the routing map correctly.
- **Audio looper — Android waveform preview stayed empty after recording, and autosave crashed while exporting WAV files**.
- **Audio looper — full French/English localization**. Waveform placeholders, every transport-strip tooltip, and every status label now go through the localization system.

## [2.12.2] - 2026-04-12

### Added
- **Audio looper — Android now records only the cabled instruments**, matching Linux. Previously every active clip recorded the full master mix, so overdubs re-captured their own playback and there was no way to isolate a single instrument. Keyboards, drum generators, and the theremin are supported as cabled sources on Android; stylophone and vocoder are still routed through the master mix.

## [2.12.1] - 2026-04-12

### Added
- **Audio looper — CC assign button**. The audio looper slot now exposes its own CC mapping button in the header, with "Loop Button" (arm/record/play/overdub cycle) and "Stop" as assignable targets.

### Fixed
- **Rack modules stayed silent until scrolled into view**. Audio-critical setup — session registration, native DSP allocation, Android audio bus wiring — used to run only when a slot widget became visible, so a CC-mapped "start" action aimed at an off-screen module would launch the transport but produce silence. All module types now initialise as soon as the project loads, regardless of scroll position: drum generators, MIDI looper, audio looper, GFPA audio effects, stylophone and theremin.

## [2.12.0] - 2026-04-11

### Added
- **Audio Looper (PCM)**. New rack module that records and loops stereo audio in real time with bar-sync, overdub, and reverse. Cable any instrument (or VST3 plugin) into its audio inputs, press the loop button, and the loop follows the transport. Only cabled sources are recorded — no metronome or unwanted bleed. Single-button workflow mirrors the MIDI looper: idle → arm → record → play → overdub → play.
- **Bar-synced recording**. In bar-sync mode, recording starts exactly on the next downbeat and the loop is padded to the next bar boundary on stop, so every loop is whole-bar aligned. A free-form mode is available per clip for non-quantised recording.
- **Waveform preview** on the looper card with playback head and recording progress.
- **Loops persist with the project**. Audio is saved as sidecar WAV files next to the project and restored on reopen.
- **CC mapping** for the loop button and stop actions.
- **Vocoder can now be recorded into the looper or processed by a GFPA effect** on Linux/macOS.
- **Drum bleed into keyboard captures eliminated**. Percussion (channel 10) now runs on its own dedicated synth slot so metronome clicks no longer contaminate the looper when a keyboard is cabled in.

## [2.11.0] - 2026-04-08

### Added
- **USB audio device routing on Android**. Synth output (keyboards, theremin, stylophone, vocoder) can now be directed to a specific USB audio device from the output selector. When a selected device is unplugged mid-session, playback falls back to the system default and a notification is shown. Requires Android 9 or later.
- **USB audio debug screen**. New screen under Preferences listing every audio device the Android system reports — useful for diagnosing multi-USB routing issues on unusual hardware.
- **Channel-swap macro**. A hardware CC can now be mapped to swap two instrument slots' MIDI channels, optionally taking their entire signal chain along (audio cables, Jam Mode references, CC mappings). Handy for live switching between a keyboard and a pad sound on the same controller.
- **Per-project CC mappings**. CC assignments are now saved inside the project file instead of globally. Switching projects switches the entire control surface configuration.
- **Multiple mappings per CC number**. A single knob can now control several targets at once, with six target kinds available (GM CC, system action, slot parameter, channel swap, transport, global).
- **Linux packaging**. The release workflow now produces `.rpm` (Fedora), Arch/Manjaro and Flatpak bundles alongside the existing `.deb` and `.zip`.

### Fixed
- **Debian package was missing runtime dependencies**. The `.deb` now correctly declares its dependencies against the Debian/Ubuntu package names.
- **CC mappings were lost on app restart**. Mappings were never actually read back from the project file at startup.
- **CC preferences crashed when selecting the Looper category**.
- **Android — first GF Keyboard slot was silent after startup** due to a race creating duplicate synth instances during project load.

## [2.10.0] - 2026-04-06

### Added
- **PipeWire / JACK audio backend on Linux**. GrooveForge now registers as a proper JACK client with named stereo output ports, connects automatically to the system output, and works on both PipeWire and native JACK2. Target audio latency drops from around 50 ms to under 10 ms.
- **Inter-application audio routing on Linux**. GrooveForge's audio can now be wired to and from other Linux audio applications (Ardour, Bitwig, Carla) through the standard JACK graph, visible in Helvum, qjackctl, or Carla's patchbay.
- **Looper — per-track volume slider**. Each track in the looper now has its own volume control alongside mute and solo.
- **Looper — CC assignment**. The looper slot now has its own CC assign button with learn mode, supporting record/play, play/pause, stop, clear, and overdub as assignable targets. Hardware CC messages cabled to the looper are routed without any extra wiring in the rack.

### Fixed
- **GF Keyboard — second keyboard's instrument was not restored on restart**. The second keyboard always came back as piano even when the saved project said otherwise.
- **Web / WASM build was broken** by the second-keyboard fix.

## [2.9.0] - 2026-03-25

### Added
- **Drum Track Generator**. New rack module that generates transport-synced drum grooves with humanization (velocity jitter, microtiming, ghost notes), swing, and structured fills and breaks. Shares the transport clock with the looper and metronome.
- **`.gfdrum` pattern format**. A readable YAML format for authoring drum patterns with step grids, per-instrument velocity and timing configuration, and support for both looping patterns and bar-by-bar sequences. Users can write their own patterns and load them into the generator.
- **Ten bundled patterns**: Classic Rock, Jazz Swing, Bossa Nova, Tight Funk, Irish Jig, Breton An Dro, Scottish Reel, Batucada, Military March, and Jazz Half-Time Shuffle. All ten were reworked from musical research for authenticity: Bonham kick hemiola on Classic Rock, authentic spang-a-lang ride on Jazz Swing, four-bar clave variation on Bossa Nova, five variations on Tight Funk (Funky Drummer, Cissy Strut, Sly Stone, synco kick, lighter), bodhrán stroke model on Irish Jig, double-kick signature on Breton An Dro, eighth-note hi-hat and backbeat on Scottish Reel, a five-voice ensemble with agogô, tamborim chamada and repique call on Batucada.
- **Drum Generator — time signature auto-sync**. Loading a drum pattern while the transport is stopped now sets the time signature to match the pattern (6/8 for Bossa Nova, 4/4 for Rock, and so on) so the metronome and bar counter stay in sync with the feel.
- **Drum Generator — parameter autosave**. Swing, humanization, soundfont, pattern selection, count-in type and fill frequency are now saved with the project.
- **GitHub Pages documentation site**. Static marketing and handbook published at the project's site root, including a getting-started guide, module walkthroughs, MIDI FX and VST3 documentation, and guides for the `.gfdrum` and `.gfpd` formats. French mirror at `/fr/` with full EN/FR language switch.
- **Flutter WASM demo** moved under `/demo/`.

### Fixed
- **Web / WASM — drum notes sounded like piano**. The browser audio bridge ignored the drum bank and always used melodic samples, so the drum generator and GM percussion both played piano notes. Drums now use a proper TR-808 kit in the browser.
