...
### Changed
- Audio Harmonizer: a **CHORD IN** jack. Patch a keyboard's CHORD OUT into it and the chord you play sets the voices outright — voice 1 takes the first chord tone above the note you sing, voice 2 the second, and so on — instead of holding the dialled intervals. The chord holds when you let the keys go, so the harmony stays put between phrases, and the voice lanes grey out because the chord is setting them. A chord with three tones voices three parts however many the Voices setting asks for, so the setting acts as a ceiling rather than doubling a note at random. A patched chord takes precedence over Scale Lock, being the more specific instruction.
...
- Android: plugging in a USB-C DAC could silence every app on the phone until a reboot. Routing a low-latency (MMAP) stream to a USB output crashes this device's audio server outright — its sound-dose meter takes two locks in the wrong order, and the vendor ships that check as a fatal assertion — so GrooveForge now spots the crash loop and reopens on the legacy audio path, which has no MMAP thread to trip it. Latency rises on that output only, and the fast path is tried again whenever the output device changes.
...
### Architecture
- `ChordMatch` now exposes the chord's own tones alongside the scale it implies. The two are not interchangeable for a harmonizer: snapping a voice to the implied scale can land it on a passing note that clashes with the chord actually being played, where snapping to the chord tones cannot.
