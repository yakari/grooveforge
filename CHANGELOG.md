...
### Fixed
- **Arpeggiator: added 1/64, 1/16T and 1/32T divisions** — the Division selector now offers 9 options (1/4, 1/8, 1/16, 1/32, 1/64, 1/4T, 1/8T, 1/16T, 1/32T). At 120 BPM 1/64 = 31 ms/step and 1/32T = 42 ms/step, making wide-range arpeggios practical at normal tempos.
- **Arpeggiator: piano view no longer scrolls to arp step notes** — the virtual piano's `didUpdateWidget` auto-scroll now only follows notes the user is actively pressing (`_pointerNote`). Autonomous arp steps cycling through octaves no longer steal the viewport mid-performance. Harmonizer and Chord Expand root-note scroll behaviour is preserved: the user-pressed root is always among the added notes and is used as the scroll target.
- **Arpeggiator: keys visually stuck after glissando** — dragging quickly across the on-screen piano left keys highlighted in `activeNotes` permanently. Two root causes: (1) `ArpeggiateNode._handleUserNoteOn` called `_fireStep`, which added the pitch to the `_arpNoteOns` sentinel set; the sentinel was never consumed when the step was fired inline (not via the tick-merge path), so a return stroke in a glissando would misidentify the user's note-on as an arp-generated event, skip `_handleUserNoteOn`, and leave the release unable to emit a note-off. (2) `_onNotePressed` passed all events from `_applyMidiChain` to `engine.playNote` regardless of type, including gate note-offs that tick() injected into the same batch, adding stale pitches to `activeNotes`. Fixes: clear the `_arpNoteOns` sentinel immediately after inline `_fireStep` in `_handleUserNoteOn`; filter `_onNotePressed` and `_onNoteReleased` to only call `playNote`/`stopNote` for matching event types.

### Added
- **Transposer MIDI FX plugin** (`com.grooveforge.transposer`) in "Add Plugin" menu: shifts all incoming MIDI notes by ±24 semitones. The virtual piano viewport does not scroll to the transposed output pitch — the scroll logic only follows physically pressed keys (`_pointerNote`), which remain at the pre-transposition pitch.
- **Velocity Curve MIDI FX plugin** (`com.grooveforge.velocity_curve`) in "Add Plugin" menu: remaps note-on velocities using three curve modes. **Power** (default): v_out = (v_in/127)^exp × 127 where the exponent spans 0.25–4.0 (soft response → linear → hard response via a single Amount knob). **Sigmoid**: logistic S-curve centred at velocity 64; higher Amount = steeper transition. **Fixed**: replaces all velocities with a constant value (velocity 1–127). Note-offs and non-note events always pass through unchanged.
- **Gate MIDI FX plugin** (`com.grooveforge.gate`) in "Add Plugin" menu: filters out notes that fall outside a configurable velocity window or pitch range. Four parameters: Min Vel / Max Vel (0–127) and Min Note / Max Note (0–127). Suppressed note-ons are tracked so the matching note-off is also suppressed, preventing stuck notes when parameters change while notes are held. Useful for zone splits, ghost-note rejection, and velocity-window routing.
- **`VelocityCurveNode`** built-in MIDI node (`type: velocity_curve`): pure-Dart implementation of the three curve algorithms (power, sigmoid, fixed). Fast path at the linear centre (amount ≈ 0.5 in power mode) returns the incoming event list unchanged.
- **`GateNode` pitch-range filter**: existing `GateNode` extended with `maxVelocity`, `minPitch`, and `maxPitch` parameters in addition to the existing `minVelocity`. Fast path exits immediately when all four parameters are at their defaults (gate fully open). Backward-compatible: old `.gfpd` files that only bind `minVelocity` continue to work — the three new parameters stay at their defaults (no filtering).

### Added
- **Arpeggiator MIDI FX plugin** (`com.grooveforge.arpeggiator`) in "Add Plugin" menu: arpeggiates held notes in a rhythmic sequence. Incoming note-ons are suppressed and replaced by autonomously timed individual notes. Parameters: Pattern (Up / Down / UpDown / DownUp / AsPlayed / Random), Division (1/4, 1/8, 1/16, 1/32, 1/4T, 1/8T), Gate (10–100% of step duration), Octaves (1–3). Plays during live performance regardless of transport state.
- **`ArpeggiateNode`** built-in MIDI node (`type: arpeggiate`): wall-clock timing via `DateTime.now().microsecondsSinceEpoch` for transport-independent playback. Uses `_arpNoteOns` / `_arpNoteOffs` per-channel sets to distinguish arp-generated events from user events in the `tick()` / `processMidi()` feedback loop. First arp step fires immediately on key press with no perceptible latency.
- **`RackState._midiFxTicker`**: 10 ms `Timer.periodic` that drives arpeggiator (and any future time-based MIDI FX) ticks on all active instrument channels even when no user events are arriving — enables sustained arpeggios when holding a chord.
